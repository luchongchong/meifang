...
{	
    ...
    if (empty($_REQUEST['account_type']) || !in_array($_REQUEST['account_type'],
        array('user_money', 'frozen_money', 'sample_money','rank_points', 'pay_points')))
    {
        $account_type = '';
    }
    else
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $sample_money   = floatval($_POST['add_sub_sample_money']) * abs(floatval($_POST['sample_money']));
    ...
    if ($user_money == 0 && $frozen_money == 0 && $rank_points == 0 && $pay_points == 0 && $sample_money == 0)
    {
        sys_msg($_LANG['no_account_change']);
    }

    /* 保存 */
    log_account_change($user_id, $user_money, $frozen_money, $sample_money, $rank_points, $pay_points, $change_desc, ACT_ADJUSTING);
...
function get_accountlist($user_id, $account_type = '')
{
    ...
    if (in_array($account_type, array('user_money', 'frozen_money','sample_money', 'rank_points', 'pay_points')))
    {
