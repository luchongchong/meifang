...
$exc = new exchange($ecs->table("bank_info"), $db, 'id', 'bank_cost_name');
...
if ($_REQUEST['act'] == 'manage')
{
    ...
    $smarty->assign('action_link',      array('text' => '添加费用名称', 'href' => 'bank_cost_name.php?act=add'));

    $smarty->display('bank_cost_name.htm');
}
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('action_link', array('href'=>'bank_cost_name.php?act=manage', 'text' => '费用名称'));
    $smarty->assign('action',      'add');
    $smarty->assign('form_act',    'insert');
    $smarty->assign('goods_type',  array('enabled' => 1));

    assign_query_info();
    $smarty->display('bank_cost_name_info.htm');
}

elseif ($_REQUEST['act'] == 'insert')
{
    $goods_type['bank_cost_name']   = $_POST['expense_account'];


    if ($db->autoExecute($ecs->table('bank_info'), $goods_type) !== false)
    {
        $links = array(array('href' => 'bank_cost_name.php?act=manage', 'text' => $_LANG['back_list']));
        sys_msg('添加费用名称成功', 0, $links);
    }
    else
    {
        sys_msg('添加费用名称失败', 1);
...
{
    $goods_type['bank_cost_name']   = sub_str($_POST['expense_account'], 60);
    $cat_id                   = intval($_POST['cat_id']);
    if ($db->autoExecute($ecs->table('bank_info'), $goods_type, 'UPDATE', "id='$cat_id'") !== false)
    {
        $links = array(array('href' => 'bank_cost_name.php?act=manage', 'text' => $_LANG['back_list']));
        sys_msg('修改费用名称成功', 0, $links);
    }
    else
    {
...
function get_goodstype()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数以及页数 */
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('bank_info')." where bank_cost_name!=''";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $filter = page_and_size($filter);

        /* 查询记录 */
        $sql = "SELECT * FROM ". $GLOBALS['ecs']->table('bank_info'). " where bank_cost_name!='' GROUP BY id " .
