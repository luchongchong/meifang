...
$exc = new exchange($ecs->table("bank_name"), $db, 'id', 'bank_name');
...
if ($_REQUEST['act'] == 'manage')
{
    ...
    $smarty->assign('action_link',      array('text' => '添加银行名称', 'href' => 'bank_name.php?act=add'));

    $smarty->display('bank_name.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('bank_name.htm'), '',
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('action_link', array('href'=>'bank_name.php?act=manage', 'text' => '银行名称'));
    $smarty->assign('action',      'add');
    $smarty->assign('form_act',    'insert');
    $smarty->assign('goods_type',  array('enabled' => 1));

    assign_query_info();
    $smarty->display('bank_name_info.htm');
}

elseif ($_REQUEST['act'] == 'insert')
{
    $goods_type['bank_name']   = $_POST['bank_name'];//银行账户简称
	$goods_type['bank_account']   = $_POST['bank_account'];//银行账号
	$goods_type['bank_open']   = $_POST['bank_open'];//开户银行
	$goods_type['bank_input']   = $_POST['bank_input'];//期初录入
	$goods_type['bank_balance']   = $_POST['bank_input'];//银行余额
    if ($db->autoExecute($ecs->table('bank_name'), $goods_type) !== false)
    {
        $links = array(array('href' => 'bank_name.php?act=manage', 'text' => $_LANG['back_list']));
        sys_msg('添加银行名称成功', 0, $links);
    }
    else
    {
        sys_msg('添加银行名称失败', 1);
...
elseif ($_REQUEST['act'] == 'edit')
{
    $goods_type =$_GET['id'];
    admin_priv('goods_type');
	$sql = "SELECT * FROM ".$GLOBALS['ecs']->table('bank_name')." where id=$goods_type";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($db->autoExecute($ecs->table('bank_name'), $goods_type, 'UPDATE', "id='$id'") !== false)
    {
        $links = array(array('href' => 'bank_name.php?act=manage', 'text' => $_LANG['back_list']));
        sys_msg('修改银行名称成功', 0, $links);
    }
    else
    {
        sys_msg('修改银行名称失败', 1);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->drop($id))
    {
        admin_log(addslashes($name), 'remove', 'bank_name');
        $url = 'bank_name.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
function get_goodstype()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数以及页数 */
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('bank_name')."";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $filter = page_and_size($filter);

        /* 查询记录 */
        $sql = "SELECT * FROM ". $GLOBALS['ecs']->table('bank_name'). " GROUP BY id " .
