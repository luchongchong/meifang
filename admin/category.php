...
if ($_REQUEST['act'] == 'update')
{
    ...
    $dat = $db->getRow("SELECT cat_name, show_in_nav,type_img FROM ". $ecs->table('category') . " WHERE cat_id = '$cat_id'");
    
   // 修改上传分类图片
		if(!empty($_FILES['cat_img']['name']))
	   	{
	   			$name=$_FILES['cat_img']['name'];
		   		$path="../images/fenlei/".$dat['type_img'];
			  	@unlink($path);
			  	clearstatcache();	
			  //图片上传
		        $tmp=strrchr($name,'.');
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    if ($cat_count == 0 && $goods_count == 0)
    {
        ...
        $cat_img=$db->getOne($sql);
        if($cat_img){
        	$path="../images/fenlei/".$cat_img;
			@unlink($path);
			clearstatcache();	
        }
