...
$exc = new exchange($ecs->table("person"), $db, 'id', 'receive_people');
...
if ($_REQUEST['act'] == 'manage')
{
    ...
    $smarty->assign('action_link',      array('text' => '添加费用相关人', 'href' => 'ecs_person.php?act=add'));

    $smarty->display('ecs_person.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('ecs_person.htm'), '',
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('action_link', array('href'=>'ecs_person.php?act=manage', 'text' => '费用相关人'));
    $smarty->assign('action',      'add');
    $smarty->assign('form_act',    'insert');
    $smarty->assign('goods_type',  array('enabled' => 1));

    assign_query_info();
    $smarty->display('ecs_person_info.htm');
}

elseif ($_REQUEST['act'] == 'insert')
{
    $goods_type['receive_people']   = $_POST['receive_people'];
 	$goods_type['handling_people']   = $_POST['handling_people'];

    if ($db->autoExecute($ecs->table('person'), $goods_type) !== false)
    {
        $links = array(array('href' => 'ecs_person.php?act=manage', 'text' => $_LANG['back_list']));
        sys_msg('添加费用相关人成功', 0, $links);
    }
    else
    {
        sys_msg('添加费用相关人成功', 1);
...
{
    $goods_type =$_GET['id'];
    admin_priv('ecs_person');
	$sql = "SELECT * FROM ".$GLOBALS['ecs']->table('person')." where id=$goods_type";
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($db->autoExecute($ecs->table('person'), $goods_type, 'UPDATE', "id='$cat_id'") !== false)
    {
        $links = array(array('href' => 'ecs_person.php?act=manage', 'text' => $_LANG['back_list']));
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->drop($id))
    {
        admin_log(addslashes($name), 'remove', 'ecs_person');
        $url = 'ecs_person.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
function get_goodstype()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数以及页数 */
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('person');
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $filter = page_and_size($filter);

        /* 查询记录 */
        $sql = "SELECT * FROM ". $GLOBALS['ecs']->table('person'). "GROUP BY id " .
