...
$exc = new exchange($ecs->table("subject"), $db, 'id', 'expense_account');
...
if ($_REQUEST['act'] == 'manage')
{
    ...
    $smarty->assign('action_link',      array('text' => '添加费用科目', 'href' => 'ecs_subject.php?act=add'));

    $smarty->display('ecs_subject.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('ecs_subject.htm'), '',
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('action_link', array('href'=>'ecs_subject.php?act=manage', 'text' => '费用科目'));
    $smarty->assign('action',      'add');
    $smarty->assign('form_act',    'insert');
    $smarty->assign('goods_type',  array('enabled' => 1));

    assign_query_info();
    $smarty->display('ecs_subject_info.htm');
}

elseif ($_REQUEST['act'] == 'insert')
{
    $goods_type['expense_account']   = $_POST['expense_account'];


    if ($db->autoExecute($ecs->table('subject'), $goods_type) !== false)
    {
        $links = array(array('href' => 'ecs_subject.php?act=manage', 'text' => $_LANG['back_list']));
        sys_msg('添加费用科目成功', 0, $links);
    }
    else
    {
        sys_msg('添加费用科目成功', 1);
...
elseif ($_REQUEST['act'] == 'edit')
{
    $goods_type =$_GET['id'];
    admin_priv('goods_type');
	$sql = "SELECT * FROM ".$GLOBALS['ecs']->table('subject')." where id=$goods_type";
...
{
    $goods_type['expense_account']   = sub_str($_POST['expense_account'], 60);
    $cat_id                   = intval($_POST['cat_id']);

    if ($db->autoExecute($ecs->table('subject'), $goods_type, 'UPDATE', "id='$cat_id'") !== false)
    {
        $links = array(array('href' => 'ecs_subject.php?act=manage', 'text' => $_LANG['back_list']));
        sys_msg('修改费用科目成功', 0, $links);
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if ($exc->drop($id))
    {
        admin_log(addslashes($name), 'remove', 'ecs_subject');
        $url = 'ecs_subject.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
function get_goodstype()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数以及页数 */
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('subject');
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $filter = page_and_size($filter);

        /* 查询记录 */
        $sql = "SELECT * FROM ". $GLOBALS['ecs']->table('subject'). "GROUP BY id " .
