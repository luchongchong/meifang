...
//-- 商品列表，商品回收站
/*------------------------------------------------------ */
if($_REQUEST['act'] == 'second_file'){
$smarty->display('second_file.dwt');
}
...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $style_id = empty($_REQUEST['style_id']) ? 0 : intval($_REQUEST['style_id']);
    $material_id = empty($_REQUEST['material_id']) ? 0 : intval($_REQUEST['material_id']);
    ...
    //获得分利策略的选项
    $affiliate_list=$GLOBALS['db']->getALL('SELECT * FROM '.$GLOBALS['ecs']->table('affiliate_config'));
    $smarty->assign('affiliate_list',$affiliate_list);
    //获得各类的选项
    //var_dump($cat_id);die();
    $sql = 'SELECT cat_id,cat_name ,parent_id,is_show,type_img ' .'FROM ' . $GLOBALS['ecs']->table('category') ."WHERE parent_id = '0' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC";
    $res = $GLOBALS['db']->getAll($sql);
    //print_r($res);
    /*foreach ($res AS $cat_info){
                if($cat_info['cat_name'] == CATEGROY_PLACE){
                    $other_cat_list[$cat_info['cat_id']]['string']= cat_list_type($cat_info['cat_id'],$cat_id);
                    $other_cat_list[$cat_info['cat_id']]['name']= $cat_info['cat_name'];
                    $other_cat_list[$cat_info['cat_id']]['tj_name']='cat_id';
                }else if($cat_info['cat_name'] == CATEGROY_STYLE){
                    $other_cat_list[$cat_info['cat_id']]['string']= cat_list_type($cat_info['cat_id'],  0, true, 0,$style_id);
                    $other_cat_list[$cat_info['cat_id']]['name']= $cat_info['cat_name'];
                    $other_cat_list[$cat_info['cat_id']]['tj_name']='style_id';
                }else if ($cat_info['cat_name'] == CATEGROY_MATERIAL){
                    $other_cat_list[$cat_info['cat_id']]['string']= cat_list_type($cat_info['cat_id'],0, true, 0,0,$material_id);
                    $other_cat_list[$cat_info['cat_id']]['name']= $cat_info['cat_name'];
                    $other_cat_list[$cat_info['cat_id']]['tj_name']='material_id';
                }
    }*/
    $smarty->assign('other_cat_list', $other_cat_list);
    $smarty->assign('cat_list',     cat_list(0, $cat_id));
    //print_r($other_cat_list);
   // $smarty->assign('cat_list',     cat_list_type(0));
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
   ...
   include_once(ROOT_PATH . 'includes/fckeditor/fckeditor.php'); // 包含 html editor 类文件 // 包含 html editor 类文件
    ...
    $res = $GLOBALS['db']->getAll($sql);
    foreach ($res AS $cat_info){
        $other_cat_list[$cat_info['cat_id']]['string']= cat_list_type($cat_info['cat_id']);
        $other_cat_list[$cat_info['cat_id']]['name']= $cat_info['cat_name'];
        if($cat_info['cat_name'] == CATEGROY_PLACE){
            $other_cat_list[$cat_info['cat_id']]['tj_name']='cat_id';
        }else if($cat_info['cat_name'] == CATEGROY_STYLE){
    ...
    else
    {
        ...
        if ($_REQUEST['act'] == 'copy')
        {
            ...
            $sql = "SELECT 0 AS goods_id, attr_id, attr_value, attr_price,cost_price " .
        ...
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show,type_img ' .'FROM ' . $GLOBALS['ecs']->table('category') ."WHERE parent_id = '0' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC";
        $res = $GLOBALS['db']->getAll($sql);
        $sql = "SELECT cat_id,style_id,material_id FROM " . $ecs->table('goods') . " WHERE goods_id = '$_REQUEST[goods_id]'";
        $god_info=$GLOBALS['db']->getRow($sql);
        foreach ($res AS $cat_info){
            if($cat_info['cat_name'] == CATEGROY_PLACE){
                $other_cat_list[$cat_info['cat_id']]['string']= cat_list_type($cat_info['cat_id'],$god_info['cat_id']);
                $other_cat_list[$cat_info['cat_id']]['name']= $cat_info['cat_name'];
                $other_cat_list[$cat_info['cat_id']]['tj_name']='cat_id';
            }else if($cat_info['cat_name'] == CATEGROY_STYLE){
                $other_cat_list[$cat_info['cat_id']]['string']= cat_list_type($cat_info['cat_id'],  0, true, 0,$god_info['style_id']);
                $other_cat_list[$cat_info['cat_id']]['name']= $cat_info['cat_name'];
                $other_cat_list[$cat_info['cat_id']]['tj_name']='style_id';
            }else if ($cat_info['cat_name'] == CATEGROY_MATERIAL){
                $other_cat_list[$cat_info['cat_id']]['string']= cat_list_type($cat_info['cat_id'],0, true, 0,0,$god_info['material_id']);
                $other_cat_list[$cat_info['cat_id']]['name']= $cat_info['cat_name'];
                $other_cat_list[$cat_info['cat_id']]['tj_name']='material_id';
            }
        }
        //var_dump($other_cat_list);die();
//         // 扩展分类
       ...
       foreach ($goods['other_cat'] AS $cat_id)
        ...
        else
        {
            ...
            foreach ($img_list as $key => $gallery_img)
            {
                $img_list[$key]['img_url'] = '../' . (empty($gallery_img['img_url']) ? $gallery_img['img_url'] : $gallery_img['img_url']);
...
//     /* 创建 html editor */
    ...
    create_html_editor3('goods_desc', $goods['goods_desc']);//ykq_update_160603编辑器批量上传图片
    ...
    $sql = "SELECT id,theme FROM ".$ecs->table('yd_subject');
    $smarty->assign('theme',$db->getAll($sql));
    ...
    $sql = "SELECT is_sample FROM " . $ecs->table('goods') . " WHERE goods_id = '$_REQUEST[goods_id]'";
    $is_sample=$GLOBALS['db']->getOne($sql);
  
    $smarty->assign('is_sample',  $is_sample);


	/*----20160909商品类型图片显示----*/
   /* $goods_attr_list = $GLOBALS['db']->getAll("select attr_lianjie,attr_img,goods_attr_id from ".$GLOBALS['ecs']->table('goods_attr')." where goods_id=".$_REQUEST['goods_id']);
    foreach ($goods_attr_list as $key => $attr_info) {
        if(empty($attr_info['attr_img'])){
                unset($goods_attr_list[$key]);
        }
    }
    $smarty->assign('goods_attr_list',$goods_attr_list);
...
elseif($_REQUEST['act'] == 'color_type' ){
   
//        /*----20160909商品类型图片显示----*/

    $goods_attr_list = $GLOBALS['db']->getRow("select * from ".$GLOBALS['ecs']->table('goods')." where goods_id=".$_REQUEST['gid']);
    /* 图片列表 */
    $sql = "SELECT * FROM " . $ecs->table('goods_gallery') . " WHERE goods_id = '$_REQUEST[gid]'";
    $img_list = $db->getAll($sql);
    /* 格式化相册图片路径 */
    if (isset($GLOBALS['shop_id']) && ($GLOBALS['shop_id'] > 0))
    {
...
//        if(empty($attr_info['attr_img'])){
//            unset($goods_attr_list[$key]);
//            //$goods_attr_list[$key]['attr_img'] = "../".$attr_info['attr_img'];
//        }
//    }
//
//    $goods_info = $GLOBALS['db']->getRow("select goods_type,goods_sn from ".$GLOBALS['ecs']->table('goods')." where goods_id=".$_REQUEST['gid']);
//    $smarty->assign('goods_sn',$goods_info['goods_sn']);
    $smarty->assign('goods_attr_list',$goods_attr_list);
    $smarty->assign('goods_id',$_REQUEST['gid']);
//    $smarty->assign('goods_type_list', goods_type_list($goods_info['goods_type']));
//    /*------20160909END---*/
    $smarty->assign('img_list', $img_list);
    $smarty->assign('thumb_width', $_CFG['thumb_width']);
    $smarty->assign('thumb_height', $_CFG['thumb_height']);
    $smarty->display("goods_color_type.htm");
}
...
{	
	//var_dump($_POST['attr_value_list']);exit;
    $code = empty($_REQUEST['extension_code']) ? '' : trim($_REQUEST['extension_code']);
    /* 是否处理缩略图 */
    ...
    /* 处理商品数据 */
    $goods_valuation = isset($_POST['goods_valuation']) ? $_POST['goods_valuation'] : 0;//ykq_add商品计价
    ...
    $style_id   = empty($_POST['style_id'])?'' : intval($_POST['style_id']);
    $material_id= empty($_POST['material_id'])?'':intval($_POST['material_id']);
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id,style_id,material_id, brand_id, shop_price, market_price,cost_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot,is_sample, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id,affiliate_id,goods_valuation)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id','$style_id','$material_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$cost_price','$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot','$is_sample', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id',$affiliate_id,'$goods_valuation')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id,style_id,material_id,  brand_id, shop_price, market_price, cost_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot,is_sample, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,affiliate_id,goods_valuation)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id','$style_id','$material_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$cost_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot','$is_sample', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral',$affiliate_id,'$goods_valuation')";
        }
    }
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "goods_sn = '$goods_sn', " .
        		"goods_valuation = '$goods_valuation', " .
                "cat_id = '$catgory_id', " .
                "style_id ='$style_id',  " .
        ...
        if ($code != '')
        {
            $sql .= "is_real=1, extension_code='$code', ";
        }
    ...
    {	
    	 ...
    	 * 上传图片重组
    	 * */
    	foreach ($_FILES['attr_img'] as $kk=>$vv){
    		foreach ($vv as $kkk=>$vvv){
    			$attr_img_list[$kkk][$kk] = $vvv;
    		}
    	}
        ...
        if(isset($_POST['attr_id_list']))
        {
            ...
            {	 
                ...
                $cost_price = $_POST['cost_price_list'][$key];
                $attr_lianjie = $_POST['attr_lianjie'][$key];
                $attr_img = $attr_img_list[$key]['name'];
                $attr_tmp_name =$attr_img_list[$key]['tmp_name'];
                ...
                if (!empty($attr_value))
                {
                    if (isset($goods_attr_list[$attr_id][$attr_value]))
                    {
                        ...
                        $goods_attr_list[$attr_id][$attr_value]['tmp_name'] = $attr_tmp_name;
                        $goods_attr_list[$attr_id][$attr_value]['attr_lianjie'] = $attr_lianjie;
                    }
                    else
                    {
                        // 如果原来没有，标记为新增
        ...
        {	
            ...
            {	
            	$new_update = " SET attr_price = '$info[attr_price]' , cost_price = '$info[cost_price]' , attr_lianjie = '$info[attr_lianjie]'";
            	if(!empty($info['attr_img']) ){
            		//判断是否填写商品货号 用商品货号赋值图片文件路径
            		if($_POST['goods_sn'])
            		{
            			$goods_sn=$_POST['goods_sn'];
            		}
            		...
            		$info['attr_img'] = change_attr_img($info['sign'],$info['goods_attr_id'], $info['attr_img'], $info['tmp_name'],$goods_sn);
            		$new_update .= ", attr_img = '$info[attr_img]'";
            	}
            	
            	
                if ($info['sign'] == 'insert')
                {
                    $sql = "INSERT INTO " .$ecs->table('goods_attr'). " (attr_id, goods_id, attr_value, attr_price,cost_price,attr_img,attr_lianjie)".
                            "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]','$info[cost_price]','$info[attr_img]','$info[attr_lianjie]')";
                }
                elseif ($info['sign'] == 'update')
                {
                    $sql = "UPDATE " .$ecs->table('goods_attr'). $new_update. " WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                }
                else
                {
    ...
    {
    	$_POST['other_cat'][$key]=$_POST['cat_id'];
    ...
    /* 批量上传图片入库 */
    if (isset($_POST['img_urlA']))
    
    {
    
        $img_descA=$_POST['img_descA'];
    
        foreach ($_POST['img_urlA'] AS $img_urlA_id => $img_urlA)
    
        {
    
            $sql = "INSERT INTO " . $ecs->table('goods_gallery') . " (goods_id, img_url, img_desc,thumb_url, img_original) " ."VALUES ('$goods_id', '$img_urlA', '', '".preg_replace('/goods_img/', 'thumb_img', $img_urlA, 1)."', '".preg_replace('/goods_img/', 'source_img', $img_urlA, 1)."')";
            $db->query($sql);
    ...
    /* 处理相册图片 */
    handle_gallery_image($goods_id, $_FILES['img_url'], $_POST['img_desc'],$_POST['link_url'], $_POST['img_file']);
    ...
    if (!$is_insert && isset($_POST['old_img_desc']))
    {
        ...
        {
            $sql = "UPDATE " . $ecs->table('goods_gallery') . " SET img_desc = '$img_desc',link_url='$link_url' WHERE img_id = '$img_id' LIMIT 1";
            $db->query($sql);
        }
    }
 ...
 *   time:20170110
 * */
elseif($_REQUEST['act'] == 'yangben_image'){

    //更新操作
    $version_number = $_REQUEST['version_number'];  //版本编号
    $technology     = $_REQUEST['technology'];      //工艺特点
    $intro          = $_REQUEST['intro'];           //版本介绍
    $place          = $_REQUEST['place'];          //适合场所
    ...
    /* 编辑时处理相册图片链接 */
    if (!$is_insert && isset($_POST['old_img_link_url']))
    {
        foreach ($_POST['old_img_link_url'] AS $img_id => $link_url)
        {
            $sql = "UPDATE " . $ecs->table('goods_gallery') . " SET link_url = '$link_url' WHERE img_id = '$img_id' LIMIT 1";
            $db->query($sql);
        }
    }
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['type']))
    {
        ...
        /*设置为样本*/
        }elseif($_POST['type'] == 'sample'){
        	/* 检查权限 */
        	admin_priv('goods_manage');
        	update_goods($goods_id, 'is_sample', '1');
        }elseif($_POST['type'] == 'not_sample'){
        	/* 检查权限 */
        	admin_priv('goods_manage');
        	update_goods($goods_id, 'is_sample', '0');
        }
...
//-- 删除样本图片
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'drop_sample_image')
{
    check_authz_json('goods_manage');
...
elseif ($_REQUEST['act'] == 'drop_attr_img')
{
	...
	/* 删除图片文件 */
	$sql = "SELECT attr_img " ." FROM " . $GLOBALS['ecs']->table('goods_attr') .
			" WHERE goods_attr_id = '$goods_attr_id'";
	$row = $GLOBALS['db']->getRow($sql);

	if ($row['attr_img'] != '' && is_file('../' . $row['attr_img']))
	{
		@unlink('../' . $row['attr_img']);
	}
	...
	/* 删除数据 */
	$sql = "DELETE FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE goods_attr_id = '$goods_attr_id' LIMIT 1";
	$GLOBALS['db']->query($sql);

	clear_cache_files();
	make_json_result($goods_attr_id);
}
 ...
 * 
 * @param string $temp_name 图片临时存储路径
 * 
 * return string
 */
//$info['attr_img'] = change_attr_img($info['sign'],$info['goods_attr_id'], $info['attr_img'], $info['tmp_name'],$goods_sn);

function change_attr_img($action,$id='',$name,$tmp_name,$yangben_name){
	//$goods_id = $GLOBALS['db']->getOne("select goods_id from ".$GLOBALS['ecs']->table('goods_attr')." where goods_attr_id = ".$id);
	
	/*$tmp = strrchr($name,'.');
	$newname = $id.$tmp;*/
    $newname = $name;
    $yangben_name = iconv('utf-8','gbk',$yangben_name);
	$attr_change_image = "../images/".$yangben_name."/";
	//$attr_change_image = "../images/".$goods_id."/";
	//$newname = time().mt_rand(100,999).$tmp;
	//$attr_change_image = "../images/".date('Ym')."/attr_img/";
	if(!file_exists($attr_change_image)) @mkdir($attr_change_image);
	$filename = $attr_change_image.$newname;
	if($action != 'insert'){
		$old_Attr_img = $GLOBALS['db']->getOne("select attr_img from " .$GLOBALS['ecs']->table('goods_attr')." where goods_attr_id = ".$id);
		if($old_Attr_img != '' && is_file('../'.$old_Attr_img)){
			@unlink('../'.$old_Attr_img);
		}
	}
