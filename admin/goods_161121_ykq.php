...
elseif($_REQUEST['act'] == 'color_type' ){
        ...
        /*----20160909商品类型图片显示----*/
    $goods_attr_list = $GLOBALS['db']->getAll("select attr_lianjie,attr_img,goods_attr_id from ".$GLOBALS['ecs']->table('goods_attr')." where goods_id=".$_REQUEST['gid']);
    foreach ($goods_attr_list as $key => $attr_info) {
        if(empty($attr_info['attr_img'])){
                unset($goods_attr_list[$key]);
...
{	
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id,style_id,material_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
    ...
    {	
        ...
        {	
            ...
            {	
            	$new_update = " SET attr_price = '$info[attr_price]' , attr_lianjie = '$info[attr_lianjie]'";
            	if(!empty($info['attr_img']) ){
            		$info['attr_img'] = change_attr_img($info['sign'],$info['goods_attr_id'], $info['attr_img'], $info['tmp_name'],$_POST['goods_name']);
                ...
                if ($info['sign'] == 'insert')
                {
                    $sql = "INSERT INTO " .$ecs->table('goods_attr'). " (attr_id, goods_id, attr_value, attr_price,attr_img,attr_lianjie)".
                            "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]','$info[attr_img]','$info[attr_lianjie]')";
                }
 ...
 /*
 *
 *20160921----批量上传花色效果图片（商品类型）
 *
 */
elseif($_REQUEST['act'] == 'yangben_image'){
    $goods_id = $_REQUEST['goods_id'];
    $yangben_name = $GLOBALS['db']->getOne("select goods_name from ".$GLOBALS['ecs']->table('goods')." where is_sample=1 and goods_id=".$goods_id);
    if(isset($_REQUEST['yangben_attr_image'])){
        foreach($_REQUEST['yangben_attr_image'] as $attr_k=>$attr_v){
            $yangben_name = iconv('utf-8','gbk',$yangben_name);
            $new_yangben_attr = "../images/".$yangben_name;
            $attr_value = basename($attr_v,".".substr(strrchr($attr_v, '.'), 1));
            if (!file_exists($new_yangben_attr)) @mkdir($new_yangben_attr);
            $new_filename = strrchr($attr_v,'/');
            $new_filename = $new_yangben_attr . $new_filename;
            if(@rename('../'.$attr_v,$new_filename)){
               $goods_attr_info = array();
               $goods_attr_info = $GLOBALS['db']->getRow("select * from ".$GLOBALS['ecs']->table('goods_attr'). " where attr_value='".$attr_value."' and goods_id=".$goods_id);
               
               if($goods_attr_info){
                $new_filename = iconv('gbk','utf-8',$new_filename);
                $new_filename = substr($new_filename,3);
                $lianjie = "image/".$goods_attr_info['goods_id']."/".$goods_attr_info['attr_value'].".dwt";
                $sql = "update ".$GLOBALS['ecs']->table('goods_attr')." set attr_img='".$new_filename."',attr_lianjie='".$lianjie."' where attr_value='".$attr_value."' and goods_id=".$goods_id;
