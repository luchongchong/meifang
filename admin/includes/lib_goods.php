...
function get_intro_list()
{
    return array(
        ...
        'is_sample'  => $GLOBALS['_LANG']['is_sample'],
 ...
 * @return  void
 */
function handle_gallery_image($goods_id, $image_files, $image_descs,$image_link_urls, $image_urls)
{
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, img_desc,link_url ,thumb_url, img_original) " .
                "VALUES ('$goods_id', '$img_url', '$img_desc','$image_link_urls[$key]' ,'$thumb_url', '$img_original')";
            $GLOBALS['db']->query($sql);
            /* 不保留商品原图的时候删除原图 */
...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price , v.cost_price, v.attr_img ,v.goods_attr_id ".
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
		...
		if($val['attr_type'] == 1 || $val['attr_type'] == 2){
			if($val['attr_img'] == ''){
				$html .= $GLOBALS['_LANG']['spec_price'].' <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />成本价格<input type="text" name="cost_price_list[]" value="' . $val['cost_price'] . '" size="5" maxlength="10" /><input type="file" name="attr_img[]" value="" />';
			}else{
				$val['attr_lianjie'] = 'image/'.$goods_id.'/'.$val['attr_value'].'.dwt';
				$html .= $GLOBALS['_LANG']['spec_price'].' <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" />成本价格<input type="text" name="cost_price_list[]" value="' . $val['cost_price'] . '" size="5" maxlength="10" /><input type="file" name="attr_img[]" value="" style="width:150px;"/><img src="../'.$val['attr_img'].'" width="60" height="60"/><font style="margin-left:1em;">链接</font><input type="text" name="attr_lianjie[]" value="'.$val['attr_lianjie'].'" size="40"/>';
			}
		}else{
			$html .= '<input type="hidden" name="attr_price_list[]" value="0" /><input type="hidden" name="cost_price_list[]" value="0" /><input type="file" name="attr_img[]" value="" style="display:none;"/><input type="text" name="attr_lianjie[]" value="" style="display:none"/>' ;
		}
       /* $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ?
            ($val['attr_img'] == ""? $GLOBALS['_LANG']['spec_price'].' <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" /><input type="file" name="attr_img[]" value="" />':$GLOBALS['_LANG']['spec_price'].' <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" /><input type="file" name="attr_img[]" value="" /><img src="../'.$val['attr_img'].'" width="60" height="60"/>') :
            '<input type="hidden" name="attr_price_list[]" value="0" /><input type="file" name="attr_img[]" value="" style="display:none;"/>' ;
		*/
        $html .= '</td></tr>';
    }
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $filter['style_id']           = empty($_REQUEST['style_id']) ? 0 : intval($_REQUEST['style_id']);
        $filter['material_id']           = empty($_REQUEST['material_id']) ? 0 : intval($_REQUEST['material_id']);
       ...
       // echo 123;die();
        ...
        $where .= $filter['style_id'] > 0 ? " AND " . 'g.style_id='.$filter['style_id']: '';
        $where .= $filter['material_id'] > 0 ? " AND " .  'g.material_id='.$filter['material_id'] : '';
