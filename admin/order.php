...
elseif(isset($_REQUEST['export'])){
	...
	if(empty($end_time)){
		...
		->setCellValue('A3', '下单日期')
		->setCellValue('B3', '采购单号')
		->setCellValue('C3', '供应商')
		//->setCellValue('D3', '需求方')
		->setCellValue('E3', '样本编号')
		//->setCellValue('F3', '产品型号')
		->setCellValue('G3', '单位')
		->setCellValue('H3', '数量')
		->setCellValue('I3', '供应商计划交期')
		->setCellValue('M3', '发货方式')
		->setCellValue('N3', '单号')
		->setCellValue('P3', '进货单家')
		->setCellValue('Q3', '进货金额')
		->setCellValue('R3', '零售价')
		->setCellValue('S3', '总价')
		;
		...
		foreach($list as $v){
			$phpExcel->setActiveSheetIndex(0)
			->setCellValue('A'.$cell_index, $v['add_time'])
			->setCellValue('B'.$cell_index, 0)
			->setCellValue('C'.$cell_index, 0)
			//->setCellValue('D'.$cell_index, $v['user_name'])
			->setCellValue('E'.$cell_index, $v['attr_name'])
		//	->setCellValue('F'.$cell_index, $v['goods_attr_name'])
			->setCellValue('G'.$cell_index, 0)
			->setCellValue('H'.$cell_index, $v['goods_number'])
			->setCellValue('I'.$cell_index, 0)
			->setCellValue('M'.$cell_index, 0)
			->setCellValue('N'.$cell_index, 0)
			->setCellValue('P'.$cell_index, 0)
			->setCellValue('Q'.$cell_index, 0)
			->setCellValue('R'.$cell_index, $v['goods_price'])
			->setCellValue('S'.$cell_index, $v['all_price']);
			$cell_index++;
		}
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('payment' == $step)
    {
        ...
        update_order($order_id, $order); //这里有问题 royallu 20170213 已解决
...
//        $user_rank  = $db->getone('select `user_rank` from'.$ecs->table('users').'where user_id='.$uid);
        ...
        sys_msg("操作成功", 0, array(array('href'=>'order.php?act=list','text' => '订单列表')));
    ...
    elseif (isset($_POST['pay']))
    {
       ...
       // var_dump($_REQUEST);exit();
        $sql = "SELECT order_sn FROM " . $ecs->table('order_info') .
            " WHERE order_id = ".$_REQUEST['order_id'] ;

        $order_sn = $db->getOne($sql);
     //   var_dump($order_sn);exit();
//        var_dump(111);exit();
        $sql1 = "UPDATE ".$ecs->table('factory')." SET pay_status = 2 where order_sn= '".$order_sn."'";
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['consignee'])
        {
            $where .= " AND u.user_name LIKE '%" . mysql_like_quote($filter['consignee']) . "%'";
        }
        if ($filter['email'])
        {
        ...
        /* 记录总数 */
      //  var_dump($filter['user_name']);exit();
//        if ($filter['user_name'])
//        {
//
//
...
//        }
        $sql = "SELECT COUNT(*)" .
            ...
            " inner JOIN " .$GLOBALS['ecs']->table('factory'). " AS f ON f.order_sn=o.order_sn ".
            " inner JOIN " .$GLOBALS['ecs']->table('users'). " AS us ON us.user_id=o.user_id ".
            " inner JOIN " .$GLOBALS['ecs']->table('users'). " AS u ON u.user_id=f.facilitator_id ". $where ;
        ...
        /* 查询 */
        $sql = "SELECT o.order_id, o.user_id,o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                    "o.pay_status, o.consignee, o.address, o.email, o.tel, o.extension_code, o.extension_id,o.pay_id,o.pay_image,u.*," .
                    "(" . order_amount_field('o.') . ") AS total_fee, " .
                    "IFNULL(us.user_name, '" .$GLOBALS['_LANG']['anonymous']. "') AS buyer ".
                " FROM " . $GLOBALS['ecs']->table('order_info') . " AS o " .
                ...
                " GROUP BY order_id ORDER BY o.add_time $filter[sort_order] ".
...
function export_order($start_time,$end_time){
	$res = "select o.order_id,o.order_sn,o.add_time ,o.pay_status,o.user_id,g.goods_id,g.goods_number,g.goods_price,g.goods_attr_id,g.rec_id from ".
			$GLOBALS['ecs']->table('order_goods'). "AS g left join ".$GLOBALS['ecs']->table('order_info'). "AS o on o.order_id=g.order_id".
			" where o.pay_status=2 and o.add_time between ".$start_time . " and ".$end_time." order by o.order_id desc";
