...
if ($_REQUEST['act'] == 'order_query')
{
    ...
    /* ?ʾģ??*/
    assign_query_info();
    $smarty->display('order_management_list.htm');
}
...
{		
    ...
    //暂时查询加盟商哪个吸粉最多
//    $sql = "select * from ecs_users where parent_id=164836 and user_rank=102";
//    $res=$db->getALL($sql);
//    var_dump($res);exit();
//    foreach ($res as $key=>$value){
//        $sql_21[$key] = "SELECT COUNT(*),user_name FROM ecs_users WHERE parent_id=".$value['user_id'];
//
//        $num_21[] = $db->getAll($sql_21[$key]);
//    }
//    var_dump( $num_21);exit();
//
...
//    $sql_21 = "SELECT COUNT(*) FROM " .$ecs->table('users'). " WHERE ".$where_con;
	...
	//print_r($order_list[sale_list_data]);
	foreach ($order_list[sale_list_data] as $key => $value){
		$a+=$value['goods_nums'];
		$b+=$value['sales_prices'];
	}
...
elseif($_REQUEST['act'] == 'delete'){
    ...
    $sql = "delete from ecs_order_goods where rec_id=".$rec_id;
    $db->query($sql);
    $link[] = array('href' => 'order_management.php?act=list', 'text' => '样本订单列表');
    sys_msg('此样册商品册除成功', 1, $link);
}
...
elseif($_REQUEST['act'] == 'review'){
    ...
    $smarty->assign('form_action','order_management.php?act=update');
...
elseif ($_REQUEST['act'] == 'download'){
       ...
       // echo ecs_iconv(EC_CHARSET, 'GB2312', $_REQUEST['start_date']. $_LANG['to'] .$_REQUEST['end_date']. $_LANG['sales_list']) . "\t\n";
        ...
        echo ecs_iconv(EC_CHARSET, 'GB2312', '销售时间') . "\t\n";
    ...
    make_json_result($smarty->fetch('order_management_list.htm'), '', array('filter' => $sale_list_data['filter'], 'page_count' => $sale_list_data['page_count']));
}
/*------------------------------------------------------ */
//-- ????????????
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('order_management_list.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
...
function order_list($is_pagination = true)
{
   ...
   // var_dump($filter);exit();
    /* 查询数据的条件 */
    $where=" WHERE oi.add_time >= '".$filter['start_date']."' AND oi.add_time < '" . ($filter['end_date'] + 86400) . "' and g.is_sample=1 ";
...
//    ";
   ...
   $sql = "SELECT COUNT(og.goods_id) FROM " .
	...
	$sql = "SELECT oi.*,og.is_audit,og.wallpaper_price,og.shipping_status,og.rec_id,og.op_remark,og.goods_id,oi.order_sn,oi.pay_status,  og.goods_sn,  replace(replace(og.goods_attr,char(10),''),char(13),'') as goods_attr, og.goods_name, og.goods_number AS goods_num, og.goods_price AS sales_price,oi.add_time AS sales_time
    ...
    foreach ($sale_list_data as $key => $item)
    {
        ...
        $sale_list_data[$key]['goods_nums'] = $sale_list_data[$key]['goods_num'];
        $sale_list_data[$key]['sales_prices'] = $item['sales_price'];
   
    }
    //var_dump($sale_list_data);exit();
