...
if ($_REQUEST['act'] == 'order_query')
{
    ...
    /* ?ʾģ??*/
    assign_query_info();
    $smarty->display('sample_order_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('sample_order_list.htm'), '', array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
}
...
function order_list($is_pagination = true)
{
    ...
    $where = " WHERE og.order_id = oi.order_id and ou.user_id = oi.user_id and os.shop_no=oi.shop_no and os.userid=ous.user_id and g.goods_id=og.goods_id and g.is_sample=1
    and oi.add_time >= '".$filter['start_date']."' AND oi.add_time < '" . ($filter['end_date'] + 86400) . "'";
    $where .=" and g.is_sample=1";
	...
	$sql = "SELECT oi.shipping_status,oi.pay_status,g.*,og.goods_id, ous.user_name, og.goods_sn, os.shop_no,oi.goods_amount, oi.invoice_no,oi.is_audit,oi.op_remark, replace(replace(og.goods_attr,char(10),''),char(13),'') as goods_attr, og.goods_name, og.goods_number AS goods_num, og.goods_price ".
