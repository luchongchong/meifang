...
/*if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    $number     = (isset($_REQUEST['number'])) ? '': trim(intval($_REQUEST['number']) );
...
if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'change_img'){
	...
	$goods_attr_id = $_POST['id'];
	$sql = 'select attr_img from '.$GLOBALS['ecs']->table('goods_attr')." where goods_attr_id=".$goods_attr_id;
...
//获得浏览历史
$smarty->assign('insert_history',insert_history());
/* 记录浏览历史 */
...
function get_linked_goods($goods_id)
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                ...
                'g.market_price, g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_valuation ' .
