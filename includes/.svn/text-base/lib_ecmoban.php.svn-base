...
function get_categories_tree_pro($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id != 157 and cat_id = '$cat_id'";
    ...
    */
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        $sql = 'SELECT cat_id,cat_name ,parent_id,is_show ' .
                ...
                "WHERE cat_id != 157 and parent_id = '$parent_id' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC";
