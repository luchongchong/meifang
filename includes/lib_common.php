 ...
 * 获取指定分类下的id
 */
function cat_list_type($cat_id = 0, $selected = 0, $re_type = true, $level = 0,$style_id = 0,$material_id = 0, $is_show_all = true)
{
    static $res = NULL;
    if ($res === NULL)
    {
    ...
    if ($re_type == true)
    {
        ...
        foreach ($options AS $var)
        {
            if($selected>0){
                ...
                $select .= '<option value="' . $var['cat_id'] . '" ';
                $select .= ($style_id == $var['cat_id']) ? "selected='ture'" : '';
                $select .= '>';
            }elseif($material_id>0){
                $select .= '<option value="' . $var['cat_id'] . '" ';
                $select .= ($material_id == $var['cat_id']) ? "selected='ture'" : '';
                $select .= '>';
            }else{
                $select .= '<option value="' . $var['cat_id'] . '" ';
                $select .= ($selected == $var['cat_id']) ? "selected='ture'" : '';
 ...
 * @return  void
 */
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $sample_money = 0 ,$rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
    ...
    $sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
            ...
            " sample_money = sample_money + ('$sample_money')," .
