...
function  get_categories_name($name=''){
    if ($name !='')
    {
        $sql = 'SELECT cat_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_name = '$name'";
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name,g.goods_name_style,g.material_id, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
 ...
 *
 */
function get_goods_list_by_id($cat_id,$num = 0, $from = 'web', $order_rule = ''){
     ...
     $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND '.
                'g.is_delete = 0 AND  cat_id='.$cat_id;
