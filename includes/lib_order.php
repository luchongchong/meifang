...
function sample_order_info($order_id, $order_sn = ''){

        /* 计算订单各种费用之和的语句 */
        $total_fee = " (og.goods_price*og.goods_number - discount + tax + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee) AS total_fee ";
        $order_id = intval($order_id);
        if ($order_id > 0)
        {
            $sql = "SELECT o.*,og.shipping_time,og.invoice_no,og.shipping_name,og.is_audit, " . $total_fee . " FROM " . $GLOBALS['ecs']->table('order_info') ." as o ".
                "inner join ecs_order_goods as og on og.order_id=o.order_id".
                " WHERE o.order_id = '$order_id' and og.is_audit=1";
        //    var_dump($sql);exit();
        }
        else
        {
            $sql = "SELECT *, " . $total_fee . "  FROM " . $GLOBALS['ecs']->table('order_info') .
                " WHERE order_sn = '$order_sn' and is_audit=1";
        }
        $order = $GLOBALS['db']->getRow($sql);

        $sql_sum="select * from ecs_order_goods where order_id=".$order_id." and is_audit=1";

    $oder_goods=$GLOBALS['db']->getAll($sql_sum);
 //   var_dump($oder_goods);exit();
    foreach ($oder_goods as $key =>$value){
        $order['s_goods_amount']+=$value['goods_price'];
    }
        ...
        if ($order)
        {
            $order['formated_goods_amount']   = price_format($order['s_goods_amount'], false);
...
function order_goods($order_id)
{
    $sql = "SELECT rec_id, goods_id, goods_name, goods_sn, market_price, goods_number, " .
            "goods_price,wallpaper_price, goods_attr, is_real, parent_id, is_gift, " .
...
//	/*--20160914银行转账修改factory表状态*/
//	if($order['pay_id'] == 4){
//		$GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('factory')." set pay_status='".$order['pay_status']."' where order_sn=".$order['order_sn']);
...
//        }
        ...
        $user['formated_sample_money'] = price_format($user['sample_money'], false);
    }
