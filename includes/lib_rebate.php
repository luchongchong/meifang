...
//    上级会员  通过购买者id查找(10%)
    ...
    $shangji_id = $result_3[0]['parent_id'];
  	...
  	$sql = "SELECT userid FROM ".$GLOBALS['ecs']->table('store')." where shop_no like '127%' and userid  = ".$result_3[0]['sell_id'];
  	$user_id = $GLOBALS['db']->getOne($sql);
  	if($user_id){
  		$shangji_id = $user_id;
  	}
    if(empty($shangji_id) || $result_3[0]['user_rank']==102){
        $shangji_id=$result_3[0]['sell_id'];
    }
