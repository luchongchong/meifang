...
switch ($act){
	case "config":
		if($_POST){
			...
			$reply_superiors = getstr($_POST['reply_superiors']);
			...
			$ret = $db->query ( 
			        ...
			        `reply_superiors`='$reply_superiors'
					 WHERE `id`=1;" );
			...
			$smarty->assign	( 'reply_superiors', $ret ['reply_superiors'] );
	...
	case "news":
		...
		if($_POST){
			...
			if($_POST['msgtype']==1){
				...
				$artInfo_f = $db->getAll("select article_id,title,content,description,file_url from ".$GLOBALS['ecs']->table('article')." where article_id in($artid)");
				if(!$artInfo){
