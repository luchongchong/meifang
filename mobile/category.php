...
if (!$smarty->is_cached('categorys.html'))
{
    /* 如果页面没有被缓存则重新获取页面的内容 */
    ...
    /*for($i=1;$i<=count($categories);$i++){
        ...
        }else if ($categories[$i]['name'] == CATEGROY_PLACE){
            $categories[$i]['tj_name']='material_id';
        }
    ...
    //if($cat_id){
    	//$sql = "SELECT *,cat_name as name FROM " . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";	
    	$sql = "SELECT *,cat_name as name FROM " . $GLOBALS['ecs']->table('category') . " WHERE parent_id = 0";
    	$categories = $GLOBALS['db']->getAll($sql);
    	$smarty->assign('categories',    $categories); // 分类树
    	$smarty->assign('current_id',    $cat_id);
    //}
    
    
    $rows=array();
    if($cat_id){
    	$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$cat_id' order by cat_id desc";
	    $parent_id = $GLOBALS['db']->getAll($sql);
	    foreach($parent_id as $key => $value){
	    	$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$value[cat_id]'";
	    	$children_id = $GLOBALS['db']->getAll($sql);
	    	$value['cat_ids']=$value['cat_id'];
	    	$value['cat_id']=$children_id;
	    	$rows[]=$value;
	    }	    
    }
    //print_r($rows);
    ...
    $sttyle_id=isset($_REQUEST['sttyle_id'])?intval($_REQUEST['sttyle_id']):0;;
    $matterial_id=isset($_REQUEST['matterial_id'])?intval($_REQUEST['matterial_id']):0;;*/
/*    if($catt_id>0){
        $parent_id=$catt_id;
        $children_list = $categories[$parent_id]['cat_id'];
        foreach ($children_list as $key=>$val){
            $children_list[$key]['xz_name']='cat_id';
        }
        $cat_id=isset($_REQUEST['cat_id'])?$_REQUEST['cat_id']:current($categories[$parent_id]['cat_id'])['id'];
    }elseif($style_id>0){
        $parent_id=$style_id;
        $children_list = $categories[$parent_id]['cat_id'];
        foreach ($children_list as $key=>$val){
            $children_list[$key]['xz_name']='sttyle_id';
        }
        $sttyle_id=isset($_REQUEST['sttyle_id'])?$_REQUEST['sttyle_id']:current($categories[$parent_id]['cat_id'])['id'];
    }elseif($material_id>0){
        
        $parent_id=$material_id;
        $children_list = $categories[$parent_id]['cat_id'];
        foreach ($children_list as $key=>$val){
            $children_list[$key]['xz_name']='matterial_id';
        }
        $matterial_id=isset($_REQUEST['matterial_id'])?$_REQUEST['matterial_id']:current($categories[$parent_id]['cat_id'])['id']; 
...
//     $children_list = $categories[$parent_id]['cat_id'];
//     //$cat_id=isset($_REQUEST['cat_id'])?isset($_REQUEST['cat_id']):current($categories)['id'];
//     $cat_children_id=isset($_REQUEST['extension_code'])?$_REQUEST['extension_code']:current($categories[$parent_id]['cat_id'])['id'];
    
    //var_dump($children_list);die();
    //$children = get_children($children_id);
    //传当前的分类id和子id
    //echo $parent_id;die();
    //echo $children_id;die;
    $smarty->assign('rows', $rows);
    $smarty->assign('parent_id', $parent_id);
    $smarty->assign('id', $id);
    $smarty->assign('cat_id', $cat_id);
    $smarty->assign('sttyle_id', $sttyle_id);
    $smarty->assign('matterial_id', $matterial_id);
    $smarty->assign('children_list', $children_list);
...
//     $smarty->assign('hot_goods',       get_category_recommend_goods('hot', $children, $brand, $price_min, $price_max, $ext));
    //echo $cat_id;die();
    $count = get_cagtegory_goods_count($cat_id,$sttyle_id,$matterial_id, $brand, $price_min, $price_max, $ext);
    ...
    $goodslist =get_goods_info_by_cat($cat_id,$sttyle_id,$matterial_id, $size, $page, $sort, $order);
    if ($_GET['act'] == 'asynclist') {
    	$sayList = array();
 ...
 * @return  integer
 */
function get_cagtegory_goods_count($cat_id,$sttyle_id,$matterial_id, $brand = 0, $min = 0, $max = 0, $ext='')
{
...
* @return  array
*/
function get_goods_info_by_cat($cat_id,$sttyle_id,$matterial_id, $size, $page, $sort, $order, $extension_code)
{
    ...
    $display = $GLOBALS['display'];
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, left(g.goods_name,10) as goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, g.shop_price AS shop_price, g.promote_price, g.goods_type,
            g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
