...
if ($_REQUEST['act'] == 'list')
{
    ...
    if (!$smarty->is_cached('exchange_list.dwt', $cache_id))
    {
        ...
        $smarty->assign('page_title',       '积分商城_美房美帮');    // 页面标题
        ...
        //$goodslist = exchange_get_goods('', $integral_min, $integral_max, $ext, $size, $page, $sort, $order);
        if($display == 'grid')
        {
...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if (!$smarty->is_cached('exchange_goods.dwt', $cache_id))
    {
        ...
        else
        {
            ...
            assign_dynamic('exchange_goods_test');
        }
    }
    $smarty->display('exchange_goods_test.dwt',$cache_id);
}
...
elseif ($_REQUEST['act'] == 'buy')
{
    ...
    if((!empty($specs)) && ($product_info['product_number'] == 0) && $product_info['product_id']!=0  && ($_CFG['use_storage'] == 1))
    {
        show_message($_LANG['eg_error_number'], array($_LANG['back_up_page']), array($back_act), 'error');
    }
