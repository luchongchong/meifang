...
if ($act == 'default')//主页面
{
	...
	$page_size = 15;
    	...
    	if($long&&$lat||!empty($province_name)||!empty($city_name)||!empty($area_name)){
    	    ...
    	    if(!empty($province_name)){
    	        ...
    	        $sql = "select ".$field." from ".$ecs->table('store')." where ".$where. " and shop_no NOT like '".$a."%' and shop_no NOT like '".$b."%'";
    	    }
    	    ...
    	    $condition = '';
    	    if($id_str){
    	    	$condition = " where store_id in(".$id_str.")";
    	    }
    	   
    	    
    	    $sql_store = "select * from ".$ecs->table('store').$condition. " and shop_no NOT like '".$a."%' and shop_no NOT like '".$b."%'";
    	    $result = $GLOBALS['db']->getAll($sql_store);
    	       ...
    	       if($allRes){
        	        ...
        	        $id_str = implode(',',$data_res[$page-1]);
        	        $condition = "store_id in(".$id_str.")";
        	        
        	        $sql_store = "select * from ".$ecs->table('store')." where ".$condition. " and shop_no NOT like '".$a."%' and shop_no NOT like '".$b."%'";
                    $result = $GLOBALS['db']->getAll($sql_store);
    	       }else{
    	        $result=$allRes;
    	       }
	...
	if($long&&$lat){	
		...
		$data_res = array_chunk($data_res, $page_size);
   		...
   		$id_str = implode(',',$data_res[$page-1]);
   		$condition = "store_id in(".$id_str.")";
   		$sql_store = "select * from ".$ecs->table('store')." where ".$condition;
   ...
   $b='087';
	if($long&&$lat||!empty($province_name)||!empty($city_name)||!empty($area_name)){
	    ...
	    if(!empty($province_name)){
	        ...
	        $sql = "select store_id  from ".$ecs->table('store')." where ".$where. " and shop_no NOT like '".$a."%' and shop_no NOT like '".$b."%'";
	    ...
	    $id_str = implode(',',$data_res[$page-1]);
	    if(!empty($id_str)){
    	    $condition = "store_id NOT in(".$id_str.")";
    	    $sql_store = "select * from ".$ecs->table('store')." where " .$condition. " and shop_no NOT like '".$a."%' and shop_no NOT like '".$b."%'";
    	    $result = $GLOBALS['db']->getAll($sql_store);
