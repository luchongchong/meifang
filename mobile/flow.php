...
if ($_REQUEST['step'] == 'add_to_cart')
{
    ...
    if (!is_numeric($goods->number) || floatval($goods->number) <= 0)
    {
        $result['error']   = 1;
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
        ...
        $smarty->assign('name_of_region',   array($_CFG['name_of_region_1'], $_CFG['name_of_region_2'], $_CFG['name_of_region_3'], $_CFG['name_of_region_4'],'街道'));
        ...
        $district_list = array();
        $street_list = array();
        $consignee_packet[]=$consignee_list;
        foreach ($consignee_packet as $region_id => $consignee)
        {
            $consignee['country']  = $consignee['country']>0  ? intval($consignee['country'])  : 1;
        ...
        /* 返回收货人页面代码 */
        $smarty->assign('real_goods_count', exist_real_goods(0, $flow_type) ? 1 : 1);
    }
    else
    {
        /*
        ...
        $consignee = array(
            ...
            'street'      => empty($_POST['street'])   ? '' :   intval($_POST['street']),
...
//     }
    ...
    $district_id = $consignee['district'];
    /* 取得配送列表 */
...
$sql = "SELECT concat(IFNULL(p.region_name, ''), '  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, '')) AS region " .
   ...
   WHERE p.`region_id`='".$consignee['province']."'  AND t.`region_id`='".$consignee['city']."'  AND d.`region_id`='".$consignee['district']."' ";
    ...
    $shipping_list = remove_array($shipping_list);
...
elseif ($_REQUEST['step'] == 'sure'){
    ...
    if($_POST['service_no']){
        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('store')."where shop_no ='".$_POST['service_no']."'";
        $res= $GLOBALS['db']->getRow($sql);        
        if($res){
            $userid_1 = $res;
        }else{
            show_message(请填写正确的服务门店编号！);
            exit;   
        }
    }else{
        show_message(请填写服务门店编号！);
        
    }
	if($_SESSION['user_id']){
    	$sql = "SELECT sell_id FROM " . $GLOBALS['ecs']->table('users')."where user_id ='".$_SESSION['user_id']."'";
        $sell_id= $GLOBALS['db']->getOne($sql);
        if($sell_id){
        	$sql = "SELECT store_id FROM " . $GLOBALS['ecs']->table('store')."where store_id ='".$sell_id."' and shop_no like '127%'";
        	$store_id= $GLOBALS['db']->getOne($sql);
        	if($store_id){
        		if($_POST['do_shop_no']){
        			
        		}else{
        			show_message('请填写施工店编号！');
        			exit;
        		}
    ...
    $province_id_1 = $userid_1['region_1'];
    $city_id_1 = $userid_1['region_2'];
    $district_id_1 = $userid_1['region_3'];
    $province = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id='$province_id_1'");
    $city = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id='$city_id_1'");
    $district = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id='$district_id_1'");
    $address_1  = $province."-";
    $address_1 .= $city."-";
    $address_1 .= $district."-";
    $address_1 .= $userid_1['address'];
    $smarty->assign('address_1', $address_1);
    $smarty->assign('inv_payee', $_POST['inv_payee']);
    $smarty->assign('fuwu', $userid_1);
    /*选择施工门店id*/
    
    if($_POST['do_shop_no']){
        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('store')."where shop_no ='".$_POST['do_shop_no']."'";
        $res= $GLOBALS['db']->getRow($sql);
        if($res){
             $userid_2 = $res;
        }else{
    ...
    $user_id_2 =($userid_2!= 0)?$userid_2:$userid_1;
    $province_id_2 = $user_id_2['region_1'];
    $city_id_2 = $user_id_2['region_2'];
    $district_id_2 = $user_id_2['region_3'];
    $province_1 = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id='$province_id_2'");
    $city_1 = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id='$city_id_2'");
    $district_1 = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id='$district_id_2'");
    $address_2 = $province_1."-";
    $address_2 .= $city_1."-";
    $address_2 .= $district_1."-";

    $address_2 .= $user_id_2['address'];
    ...
    */
    ...
    $order = array(
                    ...
                    'shop_no'         => !empty($_REQUEST['service_no'])?$_REQUEST['service_no']:'',
                    'construction_no' => !empty($_REQUEST['do_shop_no'])?$_REQUEST['do_shop_no']: $_REQUEST['service_no']
    ...
    //取出购物车商品的型号,截取型号对比商品型号的成本价
   /* foreach($cart_goods AS $aa){
    	if($aa['goods_attr']){
    		
    		print_r($aa['goods_attr']);
	        $arr = explode("型号:",$aa['goods_attr']);
	        //$arr=explode('',$arr[0]);
	        print_r($arr);
	        $xinghao=$arr[1];
	        die();
	    	$sql="SELECT cost_price FROM " . $ecs->table('goods_attr') . " WHERE goods_id=".$aa['goods_id'] ." AND attr_value ='".$xinghao."'";
	    	$cost_price=$db->getOne($sql);
	    	$sql = "UPDATE " . $ecs->table('cart') . " SET cost_price = '$cost_price' WHERE rec_id = '$aa[rec_id]'";    	
	        $db->query($sql);
    	}
    	else{
    		$sql="SELECT cost_price FROM " . $ecs->table('goods') . " WHERE goods_id=".$aa['goods_id'] ."";
	    	$cost_price=$db->getOne($sql);
	    	$cost_price;
    	}
    }*/
    ...
    if ($payment['pay_code'] == 'balance' && $order['order_amount'] > 0)
    {
        ...
        //判断商品是否是样本，如果样本，进行样本余额支付
        foreach($cart_goods as $key =>$val){
        	$val['goods_id'];
        	$sql = "SELECT `is_sample` FROM " . $GLOBALS['ecs']->table('goods')."where goods_id ='".$val['goods_id']."'";
        	$res= $GLOBALS['db']->getOne($sql);
        	
        	//判断购买是否样本
        	if($res==1){
        		
        		if ($order['order_amount'] > ($user_info['sample_money'] + $user_info['credit_line']))
		        {
		            show_message($_LANG['balance_not_enoughs']);
		        }
		        else{
		        	$order['sample_money'] = $order['order_amount'];
	        		$order['surplus']=0;
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
            ...
            "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id,cost_price) ".
    ...
    if($_POST['service_no']){
        $sql = "SELECT `userid`,`parent_id` FROM " . $GLOBALS['ecs']->table('store')."where shop_no ='".$_POST['service_no']."'";
        $res= $GLOBALS['db']->getRow($sql);
        $parent_id_1 = $res['parent_id'];
        $userid_1 = $res['userid'];
    }else{
        show_message('请填写服务门店编号！');
        exit;
    }
    $order['chose_anency_id'] =$userid_1;
    
    /*选择施工门店id*/
    if($_POST['do_shop_no']){
        $sql = "SELECT `userid`,`parent_id` FROM " . $GLOBALS['ecs']->table('store')."where shop_no ='".$_POST['do_shop_no']."'";
        $res= $GLOBALS['db']->getRow($sql);
        $parent_id_2 = $res['parent_id'];
        $userid_2 = $res['userid'];
    }
    
    $order['chose_facilitator_id'] =($userid_2!= 0)?$userid_2:$order['chose_anency_id'];
    /*查询施工门店上级服务商id*/
    $order['facilitator_id'] =($parent_id_2!= 0)?$parent_id_2:$parent_id_1;
    //var_dump($order['facilitator_id']);
    ...
    $order['parent_id'] = ($res)?$res:'196';
    $order['anency_id'] = ($res1)?$res1:'196';
    
    //余额支付 微信支付  银行卡支付 支付宝
    if($order['pay_id']==2 || $order['pay_id'] == 3 || $order['pay_id'] == 4 || $order['pay_id'] == 5){
        $res2 = $GLOBALS['db']->getAll( "select * from".$ecs->table('order_goods')."where order_id='".$order['order_id']."'");
        foreach ($res2 as $key => $value) {
            $goods_id = $value['goods_id'];
            $suppliers_id = $GLOBALS['db']->getone("select `suppliers_id` from".$ecs->table('goods')."where goods_id='".$goods_id."'");
            $rec_id = $GLOBALS['db']->getone("select `rec_id` from".$ecs->table('order_goods')."where goods_id='".$goods_id."'");
    
            $factory['suppliers_id'] =$suppliers_id;
            $factory['rec_id'] = $value['rec_id'];
            $factory['goods_name'] =$value['goods_name'];
            $factory['goods_sn'] =$value['goods_sn'];
            $factory['goods_number'] =$value['goods_number'];
            $factory['goods_attr'] =$value['goods_attr'];
            $factory['facilitator_id'] = $order['chose_facilitator_id'];
            $factory['parent_facilitator_id'] = $order['facilitator_id'];
    
            if($_POST['send_kehu'] == '1'){
                ...
                $city = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id='$city_id'");
                $district = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id='$district_id'");
                $factory['c_address'] = $province."-";
                $factory['c_address'] .= $city."-";
                $factory['c_address'] .= $district."-";
                $factory['c_address'] .= $order['address'];
                $factory['c_name'] = $order['consignee'];
                $factory['c_tel'] = $order['tel'];
                $factory['send_status'] = "1";
            }else{
                $address_id = $GLOBALS['db']->getone("select `address_id` from".$ecs->table('users')."where user_id='". $order['facilitator_id']."'");
                 
    
                if($address_id != 0){
                    $address = $GLOBALS['db']->getRow("select * from".$ecs->table('user_address')."where address_id='".$address_id."'");
                    ...
                    $factory['c_address'] .= $city."-";
                    $factory['c_address'] .= $district."-";
                    $factory['c_address'] .= $address['address'];
                    $factory['c_name'] = $address['consignee'];
                    $factory['c_tel'] = $address['tel'];
    
                }else{
    
                    $address_1 = $GLOBALS['db']->getRow("select * from".$ecs->table('users')."where user_id='". $order['facilitator_id']."'");
                    $factory['c_address'] = $address_1['msn'];
                    $factory['c_name'] = $address_1['user_name'];
                    $factory['c_tel'] = $address_1['mobile_phone'];
                }
            }

             
            $factory['user_id'] = $order['user_id'];
    
            //var_dump($address);
    
            $factory['pay_status'] = $order['pay_status'];
    
            $factory['add_time'] = $order['add_time'];
            $factory['add_time'] = $order['add_time'];
            $factory['order_sn'] = $order['order_sn'];
            $rec_id = $db->getone("SELECT * FROM " .$ecs->table('factory'). " WHERE rec_id='".$factory['rec_id']."'");
    
            if($rec_id){
                exit;
            }else{
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('factory'), $factory, 'INSERT');
            }
        }
        
        //客户下订单写入采购订单表中
        $province_id = $order['province'];//省
        $city_id = $order['city'];//市
        $district_id = $order['district'];//县
        $buy['add_time'] = $order['add_time'];//下单时间
        $buy['order_sn'] = $order['order_sn'];//订单流水
        $buy['user_id'] = $order['user_id'];
        
        //收货地址省市区
        $province = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id='$province_id'");
        $city = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id='$city_id'");
        $district = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id='$district_id'");
        $buy['address'] = $province."-";
        $buy['address'] .= $city."-";
        $buy['address'] .= $district."-";
        $buy['address'] .= $order['address'];
        $buy['name'] = $order['consignee'];
        $buy['tel'] = $order['tel'];
        $buy['facilitator_id'] = $order['chose_facilitator_id'];
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('buy'), $buy, 'INSERT');
        $buy_id = $GLOBALS['db']->insert_id();
        foreach ($res2 as $key => $value) {
            $goods_id = $value['goods_id'];
            $suppliers_id = $GLOBALS['db']->getone("select `suppliers_id` from".$ecs->table('goods')."where goods_id='".$goods_id."'");    
            $buy_good['suppliers_id'] =$suppliers_id;//工厂id
            $buy_good['buy_id'] =$buy_id;
            $buy_good['goods_id'] =$goods_id;
            $buy_good['goods_name'] =$value['goods_name'];//商品名称
            $buy_good['goods_number'] =$value['goods_number'];//商品数量
            $buy_good['goods_price'] =$value['goods_price'];//商品价格
            $buy_good['cost_price'] =$value['cost_price'];//商品价格
            $buy_good['goods_total'] =$value['goods_number']*$value['goods_price'];//商品价格
            $buy_good['goods_attr'] =$value['goods_attr'];//商品型号
            $buy_good['pay_status'] =2;//支付状态 已支付
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('buy_goods'), $buy_good, 'INSERT');
        }
...
*/
	...
	/* 处理样本余额 */
    if ($order['user_id'] > 0 && $order['sample_money'] > 0)
    {
        log_account_change($order['user_id'], 0, 0, 0, 0, sprintf($_LANG['pay_order'], $order['order_sn']),$order['sample_money'] * (-1));       
    }
    ...
    $smarty->assign('bonus', $_POST['bonus']);
    $smarty->assign('postscript',$_POST['postscript']);
    $smarty->assign('payment', $_POST['payment']);
    $smarty->assign('shipping', $_POST['shipping']);
    $smarty->assign('service_no', $_POST['service_no']);
    $smarty->assign('do_shop_no', $_POST['do_shop_no']);
    $smarty->assign('send_kehu', $_POST['send_kehu']);
    $smarty->assign('shigong', $user_id_2);
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $order_id = isset($_GET['order_id']) ? intval($_GET['order_id']) : $order_id;
    /* 订单详情 */
...
//二维数组重复元素过滤
//$array需要过滤的元素
function remove_array($array=array()){
    $tmp=array();
...
/*查询上级id*/
function  parent_id($userid){
    $sql= "SELECT parent_id FROM " . $GLOBALS['ecs']->table('users') .
...
/*查询上级加盟商id*/
function  anency_id($userid){
    $sql= "SELECT `user_id`,`parent_id`,`user_rank` FROM " . $GLOBALS['ecs']->table('users') .
    " WHERE user_id = '".$userid."'";
    $res = $GLOBALS['db']->getRow($sql);
    ...
    if($user_rank == 102){
        $anency_id = $userid;
        return $anency_id;
    }elseif($parent_id == 0||$user_rank==103){
        $anency_id = '196';
        return $anency_id;
    }else{
        $anency_id= anency_id($parent_id);
        return $anency_id;
    }

}
/*查询上级服务商id*/
function  facilitator_id($userid){
    $sql= "SELECT `user_id`,`parent_id`,`user_rank` FROM " . $GLOBALS['ecs']->table('users') .
    " WHERE user_id = '" . $userid."'";
    $res = $GLOBALS['db']->getRow($sql);
    $user_rank = $res['user_rank'];
    ...
    if($user_rank == 103){
        ...
        $facilitator_id= facilitator_id($userid);
        return $facilitator_id;
    }
