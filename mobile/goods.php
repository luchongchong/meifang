...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price')
{
    ...
    //$number     = (isset($_REQUEST['number'])) ? intval($_REQUEST['number']) : 1.0;
...
/*切换商品类型图片*/
if(!empty($_REQUEST['act']) && $_REQUEST['act'] == 'change_img'){
	$id = $_REQUEST['id'];
	$re = $GLOBALS['db']->getRow("select attr_img from ".$GLOBALS['ecs']->table('goods_attr')." where goods_attr_id = ".$id);
	if($re['attr_img'] != ''){
		$re['attr_img'] = "../".$re['attr_img'];
	}
	exit(json_encode($re));
}
...
// $goods = get_goods_info($goods_id);
 /*20160922--样本详情界面--*/
     /*   if($goods['is_sample'] == 1){

            $goods_attr_list = $GLOBALS['db']->getAll("select attr_img,attr_lianjie,attr_value from ".$GLOBALS['ecs']->table('goods_attr')." where goods_id=".$goods['goods_id']);
            $smarty->assign('goods',              $goods);
            $smarty->assign('goods_attr_list',$goods_attr_list);
            $smarty->display('yangben_info.dwt');return;
        }*/
        /*---20160922end----*/
...
{   
    ...
    else
    {
        ...
        if($goods['is_sample'] == 1){
            ...
            $goods_attr_list    =    $GLOBALS['db']->getRow("select * from ".$GLOBALS['ecs']->table('goods')." where goods_id=".$goods['goods_id']);

            $pattern_show_img   =   $GLOBALS['db']->getAll("select * from ".$GLOBALS['ecs']->table('goods_sample')." where goods_id=".$goods['goods_id']);
            foreach ($pattern_show_img as $key => $gallery_img)
            {
                $pattern_show_img[$key]['img_url'] = '../' . (empty($gallery_img['img_url']) ? $gallery_img['img_url'] : $gallery_img['img_url']);
            }
            $smarty->assign('pattern_show_img',$pattern_show_img);
            $smarty->assign('goods_attr_list',$goods_attr_list);
            $smarty->display('sample_info.dwt');return;
        }
        /*---20160922end----*/
...
if($goods['original_img']){
    ...
    $img = 'themes/miqinew/images/logo.gif';
}
...
//print_r($goods);
$url=SITE_URL.'mobile/goods4.php?id='.$goods['goods_id'];
...
$re_share = $cls_share->cls_shares($title,$url,$img,$desc,$back_url);
...
function get_zhekou($goods_id)
{
	...
	$zhekou = 0;
	 ...
	 $zhekou = (number_format(intval($goods['market_price'])/intval($goods['shop_price']),2))*10;
