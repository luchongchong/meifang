 ...
 * @return  array
 */
function  get_categories_name($name=''){
    if ($name !='')
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_name = '$name'";
        $parent_id = $GLOBALS['db']->getOne($sql);
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (!in_array($type, array('best', 'new', 'hot','sample')))
    {
        return array();
    }
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $type_goods['sample']=array();
        $data = read_static_cache('recommend_goods');
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot,g.is_sample, g.is_promote, b.brand_name,g.sort_order ' .
               ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
               ' LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
               ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1 OR g.is_sample = 1)'.
            ...
            if (!empty($goods_res))
            {
                foreach($goods_res as $data)
                {
                    ...
                    if ($data['is_sample']==1){
                        $goods_data['sample'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['sort_order']);
                    }
                    if ($data['brand_name'] != '')
                    {
        ...
        $type2lib = array('best'=>'recommend_best', 'new'=>'recommend_new', 'hot'=>'recommend_hot','sample'=>'recommend_sample');
        if (empty($type_array))
        {
        ...
        $type_merge = array_merge($type_array['new'], $type_array['best'], $type_array['hot'],$type_array['sample']);
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            if (in_array($row['goods_id'], $type_array['sample']))
            {
            	$type_goods['sample'][] = $goods[$idx];
            }
...
function get_promote_goods($cats = '')
{
    ...
    foreach ($result AS $idx => $row)
    {
        ...
        {
            $goods[$idx]['gmt_end_time'] = local_date('M d, Y H:i:s',($row['promote_end_date']+28800));
        }
        else
        {
 ...
 * @goods_id int 
 * */
function get_goods_sort($cat_id){
   if ($cat_id>0){
 ...
 * @return      array
 */
function get_cat_id_goods_list($cat_id = '', $num = '10') {
