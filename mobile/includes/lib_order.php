...
function cart_goods($type = CART_GENERAL_GOODS)
{
    ...
    $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number, " .
            "market_price, goods_price, cost_price, goods_attr, is_real, extension_code, parent_id, is_gift, is_shipping, " .
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
                "g.market_price, g.shop_price AS org_price,g.cost_price, g.promote_price, g.promote_start_date, ".
    ...
    $str=explode(',',$goods_attr_id);
    foreach($str as $key => $value){
    	$sql = "SELECT cost_price FROM " .$GLOBALS['ecs']->table('goods_attr'). " WHERE goods_attr_id = '$value'";
...
function get_cart_goods($id_ext='')    //代码修改  By  www.68ecshop.com   增加一个参数 $id_ext
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $total['market_price'] += $row['market_price'] * $row['goods_number'];
         ...
         $total['cost_price']  = $row['cost_price'];
    ...
    $total['cost_price'] = $total['cost_price'];
