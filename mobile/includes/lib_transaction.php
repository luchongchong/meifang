...
function get_profile($user_id)
{
    ...
    $info  = array();
    $infos = array();
    $sql  = "SELECT user_name,address_id, birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
             " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer,card_no,bank_name,card_name ".
    ...
    $info['address']    =get_consignee_list($user_id);
    $info['card_no'] = $infos['card_no'];
    $info['bank_name'] = $infos['bank_name'];
    $info['card_name'] = $infos['card_name'];
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sql = "SELECT order_id,shipping_status,shop_no,construction_no, order_sn,is_comment, order_status, shipping_status, pay_status, add_time, " .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'handler'        => $row['handler'],
                       'shop_no'        => $row['shop_no'],
                       'stor_name'      => get_stor_name($row['shop_no']),
...
//获取门店名字
function get_stor_name($shop_no){
    $sql="SELECT name FROM ".$GLOBALS['ecs']->table('store')."WHERE shop_no='$shop_no'";
...
//
//            }else{
//            	$order['pay_online'] = "<a href='./../weixinpay/js_api_call.php?order_id=".$order_id."'>立即微信支付</a>";
//            }
        }
    }
    else
