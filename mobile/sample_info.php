...
// if (!$smarty->is_cached('sample_info.dwt', $cache_id))
// {
    ...
    else
    {
	    if($goods['is_sample'] == 1){
	            ...
	            $pattern_show_img   =   $GLOBALS['db']->getAll("select * from ".$GLOBALS['ecs']->table('goods_gallery')." where goods_id=".$goods['goods_id']);
        ...
        $smarty->assign('cat_name',get_goods_sort($goods['cat_id']));//获得场所类型
        $smarty->assign('style_name',get_goods_sort($goods['style_id']));//获得风格类型
        $smarty->assign('material_name',get_goods_sort($goods['material_id']));//获得材质类型
...
function get_linked_goods($goods_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        print_r($arr[$row['goods_id']]['goods_thumb']);
