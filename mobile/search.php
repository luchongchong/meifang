...
else
{
    ...
    foreach ($cat as $val){
        if($val['name']=='材质'){
            ...
            $type=get_child_tree($val['id']);
        }
    }
    //传递当前位置
    $smarty->assign('lang',$_LANG);
    $smarty->assign('intor',$_REQUEST['intro']);
        /* 查询商品 */
        $sql = "SELECT g.goods_id, g.goods_name, g.market_price,g.click_count, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
                ...
                "WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in " ." AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .
                "ORDER BY $sort $order";
    	if ($sort=='salenum')
    	{
    		 $sql = "SELECT SUM(o.goods_number) as salenum, g.goods_id, g.goods_name, g.market_price,g.click_count, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
                ...
                "WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in ".
                    "AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .  " group by g.goods_type " .
                "ORDER BY $sort $order";
    	}
