...
if (empty($_GET['encode']))
{
    ...
    header("Location: searchfor.php?encode=$string\n");
...
else
{
    ...
    if (!empty($_REQUEST['keywords']))
    {
        ...
        foreach ($arr AS $key => $val)
        {
            ...
            $sc_dsad    = $_REQUEST['sc_ds'] ? " OR g.goods_desc LIKE '%$val%'" : '';
        ...
        $keywords.=' OR attr.attr_value LIKE "%'.$_REQUEST['keywords'].'%"';
        $keywords .= ')';
    ...
    /* 获得符合条件的商品总数 */
    $sql   = "SELECT COUNT(distinct g.goods_id) FROM " .$ecs->table('goods'). " AS g LEFT JOIN ".$ecs->table('goods_attr').
        " as attr ON g.goods_id=attr.goods_id WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in ".
    ...
    /* 查询商品 */
    $sql = "SELECT distinct g.goods_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot,g.is_sample, g.shop_price AS org_price, ".
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id LEFT JOIN".$ecs->table('goods_attr').
    ...
    while ($row = $db->FetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['good_lable']          = $row['good_lable'];
        $arr[$row['goods_id']]['market_price']  = price_format($row['market_price']);
        $arr[$row['goods_id']]['shop_price']    = price_format($row['shop_price']);
        $arr[$row['goods_id']]['promote_price'] = ($promote_price > 0) ? price_format($promote_price) : '';
        $arr[$row['goods_id']]['goods_brief']   = $row['goods_brief'];
        $arr[$row['goods_id']]['is_sample']   = $row['is_sample'];
    ...
    $pager = get_pager('searchfor.php', $pager['search'], $count, $page, $size);
    ...
    $smarty->assign('script_name', 'searchfor');
    $smarty->display('searchfor.dwt');
}
