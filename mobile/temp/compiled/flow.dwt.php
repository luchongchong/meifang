...
<script type="text/javascript" src="themesmobile/js/TouchSlide.js"></script>
<script type="text/javascript" src="themesmobile/mobile/js/jquery-1.4.4.min.js"></script>
<script type="text/javascript" src="themesmobile/mobile/js/ectouch.js"></script>
</head>
...
?> 
        ...
        <div class="desc new-tbl-cell"> <a style="color:#333" href="goods.php?id=<?php echo $this->_var['goods']['goods_id']; ?>" target="_blank" class="fragment">
          ...
          <div class="b"> <em class="price"><?php echo $this->_var['goods']['goods_price']; ?></em> <span style="jianju">x </span> <span class="txt"> 
            ...
            <input class="inputBg" type="number" min="1" max="1000" name="goods_number[<?php echo $this->_var['goods']['rec_id']; ?>]" id="goods_number_<?php echo $this->_var['goods']['rec_id']; ?>" value="<?php echo $this->_var['goods']['goods_number']; ?>" size="4"   onkeyup="changenum(<?php echo $this->_var['goods']['rec_id']; ?>)" onChange="changenum(<?php echo $this->_var['goods']['rec_id']; ?>)"/>
            <?php echo $this->smarty_insert_scripts(array('files'=>'transport.js')); ?> 
            <script>
            function changenum(rec_id){
            var goods_number = parseFloat(document.getElementById('goods_number_'+rec_id).value);
...
<section class="wrap"  >
  <div class="empty-cart">
    ...
    <div class="flex"> <a class="c-btn2  flex_in radius5" href="index.php" style=" background:#6bd0a2"> <i class="ico_04_b"></i> 去购物 </a> <a class="c-btn2  flex_in radius5" href="#" style=" margin-left:0.5rem"> 查看收藏夹</a> </div>
  </div>
</section>
<?php endif; ?> 
    ...
    <section class="order_box padd1 radius10" style="padding-top:0">
      <div class="in">
        <div class="table_box table_box1">
          ...
          <dl>
            <dd class="w50 b_no" ><?php echo $this->_var['lang']['detailed_address']; ?>: <?php echo $this->_var['province']; ?>-<?php echo $this->_var['city']; ?>-<?php echo $this->_var['district']; ?><?php echo htmlspecialchars($this->_var['consignee']['address']); ?> </dd>
          </dl>
          <!-- <dl>
            <dd class="w50 b_no" >
              <input type="checkbox" name="send_kehu" value="1">
                直接发送给客户
            </dd>
          </dl>
          <dl>
            <dd class="w50 b_no" >(请确认收货后及时联系加盟商)</dd>
          </dl> -->
        </div>
      </div>
    </section>
    <div class="blank3"></div>
        <section class="order_box padd1 radius10" style="padding-top:0.3rem;padding-bottom:0.3rem;margin-bottom:0.3rem; ">
        <style>
            .bespoke_box_in{
            width: 90%;
            margin:5px;
       ...
       <h2 class="nstep1_tit3">服务门店编号：</h2>
      <div class="no_service">
        <input type="text" name="service_no" value="" id="service_no" placeholder="门店号必填">
      </div>
      <div class="do_service">
        <input type="checkbox" id="do_service" value="123">
        在其他地方施工
      </div>
      <div id="do_shop" style="display:none">
        <h2 class="nstep1_tit3">施工店编号：</h2>
        <div class="no_service">
          <input type="text" name="do_shop_no" value="" id="input">
        </div>
      </div>
      <script>     
        $(function(){
         if ($.browser.msie) {
          $('#do_service:checkbox').click(function () { 
           this.blur();   
           this.focus(); 
          });   
         };
         $("#do_service").change(function() {
          if($("#do_service").attr("checked")==true) {
            $('#do_shop').css('display','block');
          }else{
            $('#do_shop').css('display','none');
            $("#input").val('');
          }
         });
        }); 
...
?>
        <!--  <p>-->
        <!--    <input name="shipping" type="radio" class="radio" id="shipping_<?php echo $this->_var['shipping']['shipping_id']; ?>" value="<?php echo $this->_var['shipping']['shipping_id']; ?>" <?php if ($this->_var['order']['need_insure']): ?>checked="true"<?php endif; ?>  supportCod="<?php echo $this->_var['shipping']['support_cod']; ?>" insure="<?php echo $this->_var['shipping']['insure']; ?>"  style="vertical-align:middle" /><label for="shipping_<?php echo $this->_var['shipping']['shipping_id']; ?>"> <?php echo $this->_var['shipping']['shipping_name']; ?> [<?php echo $this->_var['shipping']['format_shipping_fee']; ?>]</label>-->
        <!--   </p>-->
        <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
        <!--</div>-->
        <?php else: ?>
        <!--<input name="shipping"  type="radio" value = "-1" checked="checked"  style="display:none"/>-->
        <input name="shipping"  type="radio" value = "12" id="shipping_12" checked="checked"  style="display:none"/>
        <?php endif; ?>
        ...
        <dl>
          ...
          <dd class="dd2 se" id="selected2">请选择支付方式</dd>
          <i></i>
        </dl>
        <div class="dl_box" id="payment" style="display:none">
          <?php $_from = $this->_var['payment_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'payment_0_43411800_1491888578');if (count($_from)):
    foreach ($_from AS $this->_var['payment_0_43411800_1491888578']):
?>
          <p>
           <input type="radio" class="radio" name="payment" id="payment_<?php echo $this->_var['payment_0_43411800_1491888578']['pay_id']; ?>" value="<?php echo $this->_var['payment_0_43411800_1491888578']['pay_id']; ?>" <?php if ($this->_var['order']['pay_id'] == $this->_var['payment_0_43411800_1491888578']['pay_id']): ?>checked<?php endif; ?> isCod="<?php echo $this->_var['payment_0_43411800_1491888578']['is_cod']; ?>"   style="vertical-align:middle" /><label for="payment_<?php echo $this->_var['payment_0_43411800_1491888578']['pay_id']; ?>"><?php echo $this->_var['payment_0_43411800_1491888578']['pay_name']; ?> <?php if ($this->_var['payment_0_43411800_1491888578']['pay_id'] == 6): ?>[<?php echo $this->_var['user_money']; ?>]<?php endif; ?></label>
           </p>
            <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
        </div>
        <?php else: ?>
        ...
        <?php endif; ?>  
        <dl class="b_no" style="line-height: 40px;display:none">
    ...
    <div class="table_box table_box2"> 
              ...
              <input name="integral" type="number" class="input" id="ECS_INTEGRAL" oninput="changeIntegral(this.value)" value="<?php echo empty($this->_var['order']['integral']) ? '0' : $this->_var['order']['integral']; ?>" size="10" style="width:50px;"/><br/>
              <?php echo $this->_var['lang']['can_use_integral']; ?>:<?php echo empty($this->_var['your_integral']) ? '0' : $this->_var['your_integral']; ?> <?php echo $this->_var['points_name']; ?>，<?php echo $this->_var['lang']['noworder_can_integral']; ?><?php echo $this->_var['order_max_integral']; ?>  <?php echo $this->_var['points_name']; ?>.
   </div>
   </section>
   <?php endif; ?>
...
?>
        ...
        <dl class="b_no" style="display:none;">
        ...
        <div class="dl_box" id="package_box" style="display:none;">
         <p> <input type="radio" class="radio"  name="pack" value="0" <?php if ($this->_var['order']['pack_id'] == 0): ?>checked="true"<?php endif; ?>  /><?php echo $this->_var['lang']['no_pack']; ?></p>
...
?>
          <p><input type="radio" class="radio" name="pack" id="pack_<?php echo $this->_var['pack']['pack_id']; ?>" value="<?php echo $this->_var['pack']['pack_id']; ?>" <?php if ($this->_var['order']['pack_id'] == $this->_var['pack']['pack_id']): ?>checked="true"<?php endif; ?>" /><label for="pack_<?php echo $this->_var['pack']['pack_id']; ?>"><?php echo $this->_var['pack']['pack_name']; ?>[<?php echo $this->_var['pack']['format_pack_fee']; ?>]</label></p>
         <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>   
        </div>
        <?php endif; ?> 
        ...
        <div class="dl_box" id="card_box" style="display:none;">
           <p> <input type="radio" class="radio"  name="card" value="0" <?php if ($this->_var['order']['card_id'] == 0): ?>checked="true"<?php endif; ?>  /><?php echo $this->_var['lang']['no_card']; ?></p>
...
?>
           <p><input type="radio" class="radio"  name="card"  id="card_<?php echo $this->_var['card']['card_id']; ?>" value="<?php echo $this->_var['card']['card_id']; ?>" <?php if ($this->_var['order']['card_id'] == $this->_var['card']['card_id']): ?>checked="true"<?php endif; ?>   /><label for="card_<?php echo $this->_var['card']['card_id']; ?>"><?php echo $this->_var['card']['card_name']; ?>[<?php echo $this->_var['card']['format_card_fee']; ?>]</label>
           </p>
            <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
        </div>
        <?php endif; ?> 
        ...
        <div class="dl_box">
         <dl>
                ...
                <dd >   
            <input placeholder="请输入订单备注" name="postscript" type="text"   size="20"  />
                 </dd>
         </dl>
         <dl>
                <dd class="c333">
...
?>
    ...
    <input type="hidden" name="step" value="sure" />
  </form>
</section>

<?php endif; ?> 
 
<?php if ($this->_var['step'] == "sure"): ?> 
...
<section class="wrap">
  <form action="flow.php" method="post" name="theForm" id="theForm" onSubmit="return checkOrderForm(this)">
  <section class="order_box padd1 radius10 sure" style="padding-top:0;padding-bottom:0;">
    <p style="padding-top:10px">本订单通过  <strong style="color:#f00"><?php echo $this->_var['fuwu']['shop_no']; ?></strong>&nbsp;[<small><?php echo $this->_var['address_1']; ?></small>] 销售</p><br>
    <p>由 <strong style="color:#f00"><?php echo $this->_var['shigong']['shop_no']; ?></strong>&nbsp;[<small> <?php echo $this->_var['address_2']; ?></small>] 施工</p><br>
    <?php if ($this->_var['send_kehu'] == "1"): ?>
    <p>订单直接快递给客户 <br>
    <?php echo $this->_var['inv_payee']; ?></p>
    <?php endif; ?>
  </section>
  <?php if ($this->_var['payment'] == 6): ?>
  <div class="blank3"></div> 
  <section class="order_box padd1 radius10 sure" style="padding-top:0;padding-bottom:0;">
  <input type="password" name="psd" placeholder="请输入密码" style="margin:10px;border:1px solid gray;width:92%;height:30px" />
  </section>
  <?php endif; ?> 
  <div class="blank3"></div>
    <input type="submit" name="submit" value="确认付款" class="c-btn3" />
    <input type="hidden" name="payment" value="<?php echo $this->_var['payment']; ?>" />
    <?php echo $this->_var['shipping']; ?>
    <input type="hidden" name="bonus" value="<?php echo $this->_var['bonus']; ?>" />
    <input type="hidden" name="postscript" value="<?php echo $this->_var['postscript']; ?>" />
    <input type="hidden" name="service_no" value="<?php echo $this->_var['service_no']; ?>" />
    <input type="hidden" name="do_shop_no" value="<?php echo $this->_var['do_shop_no']; ?>" />
    <input type="hidden" name="send_kehu" value="<?php echo $this->_var['send_kehu']; ?>" />
        ...
        <div class="cont">
          <section class="order on">
            ...
            <table width="90%" align="center" border="0" cellpadding="15" cellspacing="0" style="border:1px solid #ddd; margin:10px auto;" bgcolor="#FFFFFF">
              ...
              <tr>
                <td align="center"><td align="center"><?php echo $this->_var['pay_online']; ?></td></td>
              </tr>
              <?php else: ?>
              <tr>
                <td align="center"><img src="themesmobile/mobile/img/ok.png" class="ok" alt=""></td>
              </tr>
              <?php endif; ?>
            </table>
...
<?php endif; ?>
<?php if ($this->_var['step'] == "pay_order"): ?> 
