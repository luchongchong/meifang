...
{   
    ...
    //获取微信的信息
	$weixinInfo = $GLOBALS['db']->getRow("SELECT fake_id,nickname, headimgurl FROM ".$ecs->table('weixin_user')." WHERE ecuid = '$user_id'");
	...
	//add by jxy
    //获得下级店数量
    $intro_num = $db->getOne("SELECT count(user_id) FROM `ecs_users` WHERE introduce_id = '".$user_id."'");
    if($intro_num){
        $smarty->assign('intro_num',        $intro_num);
    }else{
        $intro_num = 0;
        $smarty->assign('intro_num',        $intro_num);
    }		
    ...
    //获得剩余积分
    $surplus_num = $db->getOne("SELECT SUM(`value`) FROM `ecs_point_log` WHERE user_id = '".$user_id."'");
    if($surplus_num){
        $smarty->assign('surplus_num',        $surplus_num);
    }else{
        $surplus_num = 0;
        $smarty->assign('surplus_num',        $surplus_num);
    }
   //获取签到总积分
	...
	$sql_qr = "SELECT `ticket`,`endtime` FROM `wxch_qr` WHERE `type` = 'tj' and `affiliate` = ".$user_id;
	$ticket = $db->getRow($sql_qr);
	//var_dump($ticket);die();
	if($ticket){
	    if($info['user_rank'] == '99'){
	        $endtime=time()+3600*24*30;
	        if($ticket['endtime']<$endtime){
	            $arr['rank'] = $info['user_rank'];
	            $arr['scene'] = $info['username'];
	            $arr['affiliate'] = $user_id;
	            add_qr($arr);
	            $sql_qr = "SELECT `ticket`,`endtime` FROM `wxch_qr` WHERE `type` = 'tj' and `affiliate` = ".$user_id;
	            $ticket = $db->getRow($sql_qr);
	        }
	    }
	    //echo $ticket['ticket'];die();
	    $qr_url = "http://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=".$ticket['ticket'];
    ...
    $smarty->display('my.dwt');
}

/*我的样本 */
elseif ($action == 'sample_manage'){

	$yangben_sum=0;
	$yangben_sum_money=0;
	//$user_id
	//样本总数,样本总额，样本款余额
	$where = " WHERE og.order_id = oi.order_id and oi.user_id = ".$user_id." and g.goods_id=og.goods_id and g.is_sample=1 and oi.order_status=1 and oi.pay_status=2";	//user_id=$user_id  //付完款才有 订单状态为支付

	$sql = "SELECT g.is_sample,g.goods_thumb,oi.user_id,og.goods_id,og.goods_sn, oi.goods_amount, oi.invoice_no, replace(replace(og.goods_attr,char(10),''),char(13),'') as goods_attr, og.goods_name, og.goods_number AS goods_num, og.goods_price ".
	...
	$sample_list = $db->getAll($sql);
	foreach ($sample_list as $key => $item)
	{
		$sql="select sample_money from " .$ecs->table('factory')."  where order_sn=".$user_id;
		$yangben_sum+=$item['goods_num'];
		$yangben_sum_money+=$item['sales_price'];
		
		//获取快递单号
		$sql="select send_sn from " .$ecs->table('factory')."  where order_sn=$item[order_sn]";
		$send_sn=$db->getOne($sql);
		$sample_list[$key]['send_sn']=$send_sn;
		$sample_list[$key]['sales_price'] = price_format($sample_list[$key]['sales_price']);
		$sample_list[$key]['sales_time']  = local_date('Y-m-d', $sample_list[$key]['sales_time']);
	}
	$yangben_sum_money= price_format($yangben_sum_money);
//	var_dump($yangben_sum_money);
//	var_dump($yangben_sum);

	$sql_u="select sample_money from ecs_users where user_id=".$user_id;
	$sample_money=$db->getOne($sql_u);
	if(empty($sample_money)){
		$sample_money=0;
	}
	//print_r($sample_list);exit();
	$smarty->assign('yangben_sum', $yangben_sum);					//样本总数
	$smarty->assign('yangben_sum_money', $yangben_sum_money);		//样本总金额
	$smarty->assign('sample_money', $sample_money);		//用户样本余额
	$smarty->assign('sample_list', $sample_list);
	$smarty->display('sample_manage.dwt');
}
