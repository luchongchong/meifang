...
{   
	...
	$info['username'] = empty($weixinInfo['nickname']) ? $info['username']:$weixinInfo['nickname'];
    $smarty->assign('info',        $info);
    //var_dump(sprintf($_LANG['your_level'], $rank['rank_name']));die();
//     $smarty->assign('user_notice', $_CFG['user_notice']);
//     $smarty->assign('prompt',      get_user_prompt($user_id));
    //获得剩余积分
...
elseif ($action == 'weixin_fans')
{
	...
	{	
		...
		foreach($arr as $vv){
			...
			$res .= "<tr><td>".$vv['user_name']."</td><td>".$vv['wx']['nickname']."</td><td>".$img."</td><td>".$rank_name."</td><td>".$vv['reg_time']."</td><td><a href='#'>查看订单</a></td></tr>";
		}
		echo $res;
		exit;
	}
...
elseif ($action == 'act_edit_password')
{
    ...
    if ($user_info  || ($_SESSION['user_id']>0 && $_SESSION['user_id'] == $user_id ))
    {
        ...
        if ( $GLOBALS['db']->query('UPDATE'.$GLOBALS['ecs']->table('users').'SET user_name="'.$user_name.'",mobile_phone="'.$user_phone.'" WHERE user_id='.$user_id))
        {
...
function add_qr($arr)
{
	...
	if($ticket)
	{
		...
		if($is_update == 'true'){
		    ...
		    $insert_sql = "UPDATE `wxch_qr` SET ticket='".$ticket."' WHERE affiliate=".$affiliate;
		}else{
