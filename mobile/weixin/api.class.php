...
$this->log($wxid.$email.$pwd.$username.$_SESSION['user_id']);
...
$this->log("register");
$this->log($GLOBALS['err']->last_message());
...
$this->log("select ecuid from ".$GLOBALS['ecs']->table('weixin_user')." where ecuid='{$user_id}'");
$this->log("该用户已经绑定过其他微信帐号！");
...
function plusPoint($keyword,$fromUsername) {
        $db=$GLOBALS['db'];
	    $res_arr = array();
...
//		if($keyword=="g_point"){
	//
			...
			if (empty($qdok)) {
			 ...
			 $GLOBALS['err']->add($qdok);
	         return $points;
 }
function log($msg){
$file  = '/mnt/www/meifang/mobile/weixin/log.txt';//要写入文件的文件名（可以是任意文件名），如果文件不存在，将会创建一个
$content = $msg."\n";
