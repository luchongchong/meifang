...
$followInfo = $api->getFollowUserInfo($wxid);  					//weixin_user
if(!$followInfo or $followInfo['expire_in']-86400<time()){
	$info = $weixin->getUserInfo($wxid);

	if($info) $api->followUser($wxid,$info);					//insert weixin_user
}
if ($event['event'] == "subscribe") { //用户关注
	...
	if(intval($followInfo['ecuid']) == 0 && $weixinconfig['reg_type'] == 1){
		...
		//发送信息
		echo $weixin->text($weixinconfig['followmsg'].$weixinconfig['auto_reply'])->reply();//自动回复功能

		//echo $weixin->text($weixinconfig['followmsg'])->reply();//自动回复功能
		$userInfo = $api->getFollowUserInfo($wxid);
		//修改用户的上级
		$parent_id='';
		...
		$users = $GLOBALS['db']->getRow("select `user_name`,`parent_id` from ".$GLOBALS['ecs']->table('users')." where user_id='".$userInfo['ecuid']."'");

		//获得上级用户的openid
		$parent_id=$users['parent_id'];
		if($parent_id){
			$nickname=$db->getOne("SELECT `nickname` FROM " . $GLOBALS['ecs']->table('weixin_user') . " WHERE `ecuid` = ".$userInfo['ecuid']);
			...
			if($parent_id_openid){
				...
				$data=array(
					...
					"text"=>array(
						...
						"content"=> "【".$nickname ."】通过二维码关注了美房美邦,会员编号是:".$users['user_name'] .", 成为您的美美族成员！"
					)
				);
	...
	echo $weixin->text($weixinconfig['auto_reply'])->reply();//自动回复功能
	exit;
}
...
if($type == "text"){
	...
	$parent_id_openid=$db->getOne("SELECT `fake_id` FROM " . $GLOBALS['ecs']->table('weixin_user') . " WHERE `ecuid` = ".$followInfo['ecuid']);
	$content = $weixin->getRev()->getRevContent();
	$time=time();
	if($content==1){

		$newsData = array();
		$newsData[0]['Title']='【美房美邦】粉丝倍增秘籍';
		$newsData[0]['Description']="美房美邦粉丝倍增秘籍,点击标题可直接观看";
		$newsData[0]['PicUrl']='https://mmbiz.qlogo.cn/mmbiz_jpg/mG2prXHUGbVia8Icy5zqyQ0XJq6jeCockZRK8pS0ldsSKdPyul0fm6OHCgYnoTxXTFDvYvc8PPt8mzt9OYe39uQ/0?wx_fmt=jpeg';
		$newsData[0]['Url']='http://mp.weixin.qq.com/s/ZECAnW8l1222mj9daMEb8A';
		echo $weixin->news($newsData)->reply();exit;
	}
	if($content==2){
		$newsData = array();
		$newsData[0]['Title']='【美房美邦】315你敢来我就敢送';
		$newsData[0]['Description']="美房美邦315你敢来我就敢送,点击标题可直接观看";
		$newsData[0]['PicUrl']='http://mmbiz.qpic.cn/mmbiz_jpg/ZibkoksG0RKlch92SaHlrPuolrkSMWrTiaZImEveYj6D2IMf4jN6P2zzrj69FHiahHJFm28s4ysmqBfkn7Oy9dchw/0?wx_fmt=jpeg';
		$newsData[0]['Url']='https://mp.weixin.qq.com/s/LyI4feBXI-x0GkhJrwVphg';
		echo $weixin->news($newsData)->reply();exit;
	}
	if($content==3){
		$newsData = array();
		$newsData[0]['Title']='【美房美邦】下单绝顶窍门';
		$newsData[0]['Description']="美房美邦下单绝顶窍门,点击标题可直接观看";
		$newsData[0]['PicUrl']='https://mmbiz.qlogo.cn/mmbiz_jpg/mG2prXHUGbVia8Icy5zqyQ0XJq6jeCockViawJLlIntHPorpHxiaiblV3LDnlGHzcvEX0YibUuuVjZvDbA5xK7z9f9g/0?wx_fmt=jpeg';
		$newsData[0]['Url']='http://mp.weixin.qq.com/s/4ONfuPjQOqGku1Xz5n7rCw';
		echo $weixin->news($newsData)->reply();exit;
	}
	if($content==4){
		$newsData = array();
		$newsData[0]['Title']='【美房美邦】附近体验店';
		$newsData[0]['Description']="美房美邦附近体验店,点击标题可直接观看";
		$newsData[0]['PicUrl']='http://www.mfmb58.com/upload/store1.jpg';
		$newsData[0]['Url']='http://www.mfmb58.com/mobile/experience.php?act=default&user_id=104213';
		echo $weixin->news($newsData)->reply();exit;
	}
	//初始化日志
...
//$log = Log::Init($logHandler, 15);
   ...
   //$userKey = $api->keywordsToKey($keys,&$diy_type);
