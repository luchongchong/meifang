...
'account_deposit', 'account_log', 'account_detail','weixin_fans', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer');
...
elseif ($action == 'weixin_fans')
{
    ...
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('users'). " WHERE ".$where);
    ...
    $where .= ' limit '.$pager['page'].','.$pager['size'];
    ...
    $sql_all = "SELECT * FROM " .$ecs->table('users'). " WHERE ".$where_con;
    $con_all = $db->getAll($sql_all);
   
    $smarty->assign('user_rank',  $_SESSION['user_rank']);
    $smarty->assign('arr', $arr);
    $smarty->display('mypartner.dwt');
}
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        $other['id_card']       = isset($_POST['id_card'])?trim($_POST['id_card']):'';
        ...
        if (register($username, $password, $email, $other) !== false)
        {
            ...
            $smarty->assign('act','act_register');
            $smarty->display('register.dwt');
...
elseif ($action == 'profile')
{
    ...
    /*得到用户的地址*/
    $sql= 'SELECT * FROM '.$ecs->table('user_address').' WHERE address_id='.$user_info['address_id'];
    $consignee=$db->getRow($sql);
    //var_Dump($consignee);die();
    $province = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id=".$consignee['province']);
    $city = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id=".$consignee['city']);
    $district = $db->getOne("SELECT `region_name` FROM " .$ecs->table('region'). " WHERE region_id=".$consignee['district']);
    $address_name= $province."-".$city."-".$district."-".$consignee['address'];
    $smarty->assign('address_name',$address_name);
...
elseif ($action == 'address_list')
{
   ...
   //var_dump($consignee_list[0]['district']);die();
