...
{
	//获取剩余余额
	//update by jxy 20160716 
	//$surplus_amount = get_user_surplus($user_id);
	$sql = "SELECT user_money,card_no,bank_name,card_name FROM " .$ecs->table('users')." WHERE user_id = " .$user_id;
	$userInfo = array();
	$userInfo = $GLOBALS['db']->getRow($sql);

	$surplus_amount = $db->getOne($sql);	
	//update end
	if (empty($userInfo['user_money']))
	{
		$userInfo['user_money'] = 0;
	}
	$smarty->assign('surplus_amount', price_format($userInfo['user_money'], false));

	$smarty->assign('bank_info',$userInfo);
...
elseif ($action == 'account_detail')
{
	...
	//获取剩余余额
	//update by jxy 20160617
	//$surplus_amount = get_user_surplus($user_id);
	$sql = "SELECT user_money FROM " .$ecs->table('users')." WHERE user_id = " .$user_id;
	$surplus_amount = $db->getOne($sql);	
...
elseif ($action == 'account_log_c')
{
	...
	//add by jxy
	$smarty->assign('default_amount',$surplus_amount);
...
elseif ($action == 'act_account')
{
	...
	{
		/* 判断是否有足够的余额的进行退款的操作 */
		//update by lifei 20160716
		//$sur_amount = get_user_surplus($user_id);
		//$sur_amount = get_user_surplus2($user_id);
	$sql = "SELECT user_money FROM " .$ecs->table('users')." WHERE user_id = " .$user_id;
	$sur_amount = $db->getOne($sql);			
		//update end
...
elseif ($action == 'weixin_fans')
{
	...
	//会员名称搜索
	if($_GET['user_name']){
		$where .= ' and concat(`user_name`) like "%'.$_GET['user_name'].'%"';
	...
	//update by kin 20160614 昵称查询出错问题修改	start
	//$sql_zj3 = "SELECT COUNT(*) FROM " .$ecs->table('users'). " WHERE ".$where;
	if($_GET['user_name']){
	    $sql_zj3 = "SELECT COUNT(*) FROM ecs_users as u left join ecs_weixin_user as wxu on u.user_id=wxu.ecuid WHERE ".$where;
	}else{
	    $sql_zj3 = "SELECT COUNT(*) FROM " .$ecs->table('users'). " WHERE ".$where;
	}
	//update by kin 20160614 昵称查询出错问题修改	end
	...
	if($_GET['user_name']){
	    $sql_zj4 = "SELECT u.*,wxu.nickname FROM ecs_users as u left join ecs_weixin_user as wxu on u.user_id=wxu.ecuid WHERE ".$where." order by reg_time desc";
	}else{
	    $sql_zj4 = "SELECT * FROM " .$ecs->table('users'). " WHERE ".$where." order by reg_time desc";
	}
	//update by kin 20160614 昵称查询出错问题修改	end
	...
	/**
	 *	 修改ecs_users左连接weixin_user通过wxuid字段查询数据
	 * */
	foreach($arr as &$v){
	    //update by kim  on 20160612
		//$sql_zj5 = "SELECT * FROM " .$ecs->table('weixin_user'). " WHERE  ecuid = '".$v['wxuid']."'";
	    $sql_zj5 = "SELECT * FROM " .$ecs->table('weixin_user'). " WHERE  ecuid = '".$v['user_id']."'";
...
elseif ($action == 'my_income')
{
	...
	$sql1_1 = "select * from ". $ecs->table('account_log') ." where ".$where . " order by log_id desc";
...
elseif($action == 'goods_send_status'){
	...
	foreach ($list['item'] as $k8 => $v8) {
		//下单人
		$user_name = $GLOBALS['db']->getone("select `user_name` from".$ecs->table('users')."where user_id='".$v8[user_id]."'");
		$list['item'][$k8]['user_name'] = $user_name;
		//上级用户
		$superior_name = $GLOBALS['db']->getone("select `user_name` from".$ecs->table('users')."where user_id='".$v8[facilitator_id]."'");
		$list['item'][$k8]['superior_name'] = $superior_name;
		//订单总额
		$goods_amount = $GLOBALS['db']->getone("select `goods_amount` from".$ecs->table('order_info')."where order_sn='".$v8[order_sn]."'");
		$list['item'][$k8]['goods_amount'] = $goods_amount;
...
elseif($action == 'sample_list'){
    ...
    if($_POST['start_time']){
    	$start_date=strtotime($_POST['start_time']);
    }
	if($_POST['end_time']){
    ...
    if($_POST['goods_sn']){
    	$smarty->assign('goods_sn',       $_POST['goods_sn']);
    }
    
    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
    
    $sale_list_data = get_sale_list($start_date,$end_date,$_POST['user_name'],$_POST['goods_name'],$_POST['goods_sn'],$page);
    /* 赋值到模板 */
	if($page<$sale_list_data['filter']['page_count'])
	{
		$next=$page+1;
	}
	else
	{
		$next=$sale_list_data['filter']['page_count'];
	}
	if($page==1)
	{
		$up=$page;
    ...
    $smarty->assign('pager',       $sale_list_data['filter']);
    $smarty->assign('up',    $up);//上一页
    $smarty->assign('next',    $next);//下一页
    $smarty->assign('page_count',    $sale_list_data['filter']['page_count']);//最末页
...
elseif($action == 'download'){
    ...
    if($_GET['start_time']){
    	$start_date=strtotime($_GET['start_time']);
    }
	if($_GET['end_time']){
    	$end_date=strtotime($_GET['end_time']);
    }
    if($_GET['user_name']){
    	$smarty->assign('user_name',       $_GET['user_name']);
    }
    if($_GET['goods_name']){
    	$smarty->assign('goods_name',       $_GET['goods_name']);
    }
    if($_GET['goods_sn']){
    	$smarty->assign('goods_sn',       $_GET['goods_sn']);
    }
    
    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
        $goods_sales_list = get_sale_list($start_date,$end_date,$_POST['user_name'],$_POST['goods_name'],$_POST['goods_sn'],$page);
        ...
        header("Content-Disposition: attachment; filename=samples.xls");
...
//样本订单方法
function get_sale_list($start_date,$end_date,$user_name='',$goods_name='',$goods_sn='',$is_pagination){

    /* 查询数据的条件 */
     $where = " WHERE og.order_id = oi.order_id and oi.shop_no=os.shop_no and os.userid=ou.user_id and og.goods_id=ogs.goods_id". order_query_sql('finished', 'oi.') .
             "and ogs.is_sample=1 ";
	if($user_name){
		   $where.=" and ou.user_name ='$user_name'";
	    }
	...
	if($goods_sn){
	    	$where.=" and og.goods_sn='".$goods_sn."'";
	    }
    ...
    /* 分页大小 */
    $filter = page_and_size($filter);
    $sql = 'SELECT og.goods_id, og.goods_sn, oi.goods_amount, oi.invoice_no, og.goods_attr, os.shop_no, ou.user_name, og.goods_name, og.goods_number AS goods_num, og.goods_price '.
