...
if ($action == 'default')
{
	...
	$smarty->assign('info',        $user_info_c);
...
$username = isset($_POST['username']) ? trim($_POST['username']) : '';
	...
	//	print_r($re1);
	//	exit();
	if(!($re1['user_rank'] == 101 || $re1['user_rank'] == 102 || $re1['user_rank'] == 103)){
		$_SESSION['login_fail'] ++ ;
...
elseif ($action == 'weixin_fans')
{
	...
	if($_GET['start_time'] || $_GET['end_time']){
		...
		$smarty->assign('search_start', $_GET['start_time']);
		$smarty->assign('search_end', $_GET['end_time']);
	}

	//会员等级搜索
	if($_GET['user_rank']){
		$search['search_rank'] = $_GET['user_rank'];
		$page_arr['user_rank'] = $_GET['user_rank'];
		$smarty->assign('search_rank', $_GET['user_rank']);
	}

	//会员名称搜索
	if($_GET['user_name']){
		$search['search_name'] = $_GET['user_name'];
		$page_arr['user_name'] = $_GET['user_name'];
		$smarty->assign('search_name', $_GET['user_name']);
	}
	...
	$page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
	//小伙伴一览
	$below_list = $user49->get_below_list($user_id, $_SESSION['user_rank'], $page, 10, $search);

	//小伙伴数量
	$below_amount = $user49->get_below_amount($user_id, $_SESSION['user_rank'], $search);
	$record_count = $below_amount['all_below'];

	$pager  = get_pager('user_c.php', $page_arr, $record_count, $page,'10');
	$smarty->assign('act_self',  $action);
	$smarty->assign('user_rank',  $_SESSION['user_rank']);
	$smarty->assign('below_amount',  $below_amount);
	$smarty->assign('below_list',  $below_list);
...
elseif ($action == 'my_income')
{
	...
	//获取收益一览
	$income_list = $user49->get_income_list($user_id, $page, 10, $search);
	$smarty->assign('income_list', $income_list);

	//获取收益总额等
	$income_amount = $user49->get_income_amount($user_id, $search);
	$smarty -> assign('income_amount',$income_amount);
	
	$smarty->assign('user_rank',  $_SESSION['user_rank']);
	$pager  = get_pager('user_c.php', $page_arr, $income_amount['records_num'], $page,'10');
	$smarty->assign('act_self',  $action);
	$smarty->assign('pager',  $pager);
		...
		$sql="select `store_shops` from `ecs_images_upload` where `image_id`='".$image_id."'";
	     ...
	     if(file_exists($path))
	        ...
	        if(is_uploaded_file($_FILES['name']['tmp_name']))
	        {
	         if(move_uploaded_file($_FILES['name']['tmp_name'],$path))
		    {
			 echo $db->query('UPDATE ' .$ecs->table('images_upload'). "SET store_shops='$newname' WHERE image_id='$image_id'" );
			 echo "<center><br/><br/><br/><br/><br/><font color='rad' size='+2'><b>修改成功</b></font></center>";
		     echo "<script>setTimeout('location.href=\"user_c.php?act=store_photos_list\"',1000);          </script>";
		    }
			else
...
elseif ($action == 'evaluation_management')
{
	...
	echo $sql="select comment_id,user_name,content,from_unixtime(add_time,'%Y%m%d') as add_time,comment_rank,user_id from ecs_comment where user_id='".$user_id."'";
	$query=mysql_query($sql);
...
elseif ($action == 'construction_team')
{
	...
	echo $sql="select comment_id,user_name,content,from_unixtime(add_time,'%Y%m%d') as add_time,comment_rank,construction_id from ecs_comment where user_id='".$user_id."' and construction_id='".$construction_id."'";
	$query=mysql_query($sql);
	while($rs=mysql_fetch_array($query)){
		$comment_rank=$rs['comment_rank'];
		$comment+=$comment_rank;
